...
- **Impact**: `starter_trial` falls back to undefined → uses `free` → incorrect limits
...
- **Fix**: Complete migration by renaming both `plans.free` → `plans.starter_trial` AND `planLimits.free` → `planLimits.starter_trial`
...
- **Fix**: Rename `FALLBACK_LIMITS.free` → `FALLBACK_LIMITS.starter_trial` + update 2 references
...
- **Fix**: Create 3 new integration tests:
  1. `trial-expiry.integration.test.js` - Signup → time advance → subscription expired → access revoked
  2. `early-upgrade.integration.test.js` - Upgrade during trial → billing switch → trial cancellation
...
1. ✅ `src/routes/polarWebhook.js` - Remove plan downgrade on cancellation (C1)
2. ✅ `src/services/autoApprovalService.js` - Add starter_trial config (C2)
3. ✅ `src/services/costControl.js` - Complete plan migration (C3)
4. ✅ `src/services/workerNotificationService.js` - Update fallback limits (C4)
5. ✅ `src/services/stripeWebhookService.js` - Derive plan from transaction (M2)
6. ✅ `src/services/creditsService.js` - Update planLimits.free references (C3 cascade)

### Tests (3 new files)

7. 🆕 `tests/integration/trial-expiry.integration.test.js` (M1)
8. 🆕 `tests/integration/early-upgrade.integration.test.js` (M1)
9. 🆕 `tests/integration/worker-enforcement.integration.test.js` (M1)

### Documentation

10. ✅ `docs/TRIAL-MODEL.md` - Update test checklist (M1)
...
**FASE 2: MAJOR Fixes (M1-M2)** - Decouple & Test 5. M2 - stripeWebhookService.js - Derive plan dynamically 6. M1 - Create 3 integration tests 7. M1 - Update TRIAL-MODEL.md checklist

**FASE 3: Validation** 8. Run full test suite 9. GDD validation (runtime, health ≥87, drift <60) 10. Generate test evidence
...
- ✅ `plan-consistency.test.js` - 13/13 passing
- ✅ `plan-limits-integration.test.js` - 12/12 passing
- ✅ `credits-api.test.js` - 15/15 passing
- ✅ `stripeWebhooksFlow.test.js` - 17/17 passing

### Integration Tests (new, to create)

- 🆕 `trial-expiry.integration.test.js`
...
- 🆕 `early-upgrade.integration.test.js`
...
- 🆕 `worker-enforcement.integration.test.js`
...
- ❌ `multi-tenant-rls-issue-412.test.js` - NOT ADDRESSED (out of scope? create separate issue)
- ❌ `tierLimitsEnforcement.integration.test.js` - NOT ADDRESSED (covered by new worker test?)
...
### Services (6 files)
- costControl.js: Renamed plans.free → plans.starter_trial, planLimits.free → planLimits.starter_trial
- autoApprovalService.js: Added starter_trial: { hourly: 10, daily: 25 } to planLimits, updated fallback to starter_trial_fallback
- workerNotificationService.js: Renamed FALLBACK_LIMITS.free → FALLBACK_LIMITS.starter_trial
- creditsService.js: Updated planLimits.free → planLimits.starter_trial
...
### Tests (3 new)
- trial-expiry.integration.test.js: Signup → expiry → access revoked
- early-upgrade.integration.test.js: Upgrade during trial → billing switch
...
- ✅ All 6 CRITICAL + MAJOR issues resolved
- ✅ Cancellation flow matches TRIAL-MODEL.md (plan unchanged, status='canceled')
- ✅ starter_trial config exists in all services (autoApproval, costControl, workerNotification)
- ✅ No more `plans.free` or `planLimits.free` aliases (complete migration)
- ✅ stripeWebhookService derives plan dynamically

### Testing

- ✅ 100% existing tests passing (no regressions)
- ✅ 3 new integration tests created and passing
- ✅ Coverage maintains or increases
- ✅ Test checklist in TRIAL-MODEL.md updated
...
- ✅ TRIAL-MODEL.md test checklist updated
- ✅ Test evidence generated in `docs/test-evidence/review-3438481794/`
- ✅ SUMMARY.md with pattern analysis (not chronology)
