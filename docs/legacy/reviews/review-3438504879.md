...
- **Severity:** 🔴 CRITICAL (escalated from MAJOR)
...
1. ✅ `should expire trial after 30 days and revoke access`
...
2. ✅ `should maintain read-only access after expiry`
...
3. ✅ `should send expiry notification email`
   - Mock emailService
...
1. ✅ `should upgrade from starter_trial to pro mid-trial`
...
2. ✅ `should start billing immediately on early upgrade`
...
3. ✅ `should handle webhook idempotency`
...
1. ✅ `should reject roast generation for expired trial`
...
2. ✅ `should enforce tier limits during trial`
...
3. ✅ `should allow Shield moderation during trial`
...
4. ✅ `should disable all processing on trial expiry`
   - Expired trial user attempts:
     - Roast generation → Rejected
     - Comment analysis → Rejected
     - Shield action → Rejected
...
- ✅ `tests/integration/trial-expiry.integration.test.js` (~150 lines)
- ✅ `tests/integration/early-upgrade.integration.test.js` (~120 lines)
- ✅ `tests/integration/worker-enforcement.integration.test.js` (~180 lines)
