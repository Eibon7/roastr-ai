...
**Status:** ⚠️ CRITICAL - ARCHITECTURAL REFACTOR REQUIRED
...
### 🔴 MAJOR (3 issues - All Architectural)
...
1. ✅ Import production modules AFTER mocks
2. ✅ Execute production code (worker/service/handler)
3. ✅ Verify production code interacts correctly with mocks
4. ❌ Never manipulate mockSupabaseData directly in tests
5. ❌ Never call mock functions directly (call production code that calls them)
...
1. ✅ Reject roast generation for expired trial → Call worker.process()
2. ✅ Allow roast during active trial → Call worker.process()
3. ✅ Enforce tier limits (10 roasts/month) → Call planLimitsService.checkLimit()
4. ✅ Reject 11th roast with "Limit exceeded" → Call worker.process()
5. ✅ Platform limits (1 platform) → Call planLimitsService.checkPlatformLimit()
6. ✅ Shield moderation during trial → Call shieldService.executeAction()
7. ✅ Disable Shield after expiry → Call shieldService.executeAction()
8. ✅ All operations disabled on expiry → Call each service
9. ✅ Missing subscription (fail-closed) → Call worker.process()
10. ✅ Concurrent checks → Call worker.process() concurrently
11. ✅ Boundary limit (exactly 10 roasts) → Call planLimitsService
    12-18: Edge cases → All call production code
...
it('should upgrade from starter_trial to pro mid-trial', async () => {
  ...
  expect(mockSupabase.from().update).toHaveBeenCalledWith(expect.objectContaining({ plan: 'pro' }));
...
- ⚠️ `tests/integration/worker-enforcement.integration.test.js` (~484 lines → ~550 lines)
- ⚠️ `tests/integration/trial-expiry.integration.test.js` (~303 lines → ~350 lines)
- ⚠️ `tests/integration/early-upgrade.integration.test.js` (~417 lines → ~450 lines)

### Files to Create (1)

- ✅ `docs/patterns/integration-test-pattern.md` - Document correct pattern

### Files to Update (1)

- ✅ `docs/patterns/coderabbit-lessons.md` - Add Pattern #12

**Total LOC Change:** ~1,204 lines → ~1,350 lines (+146 lines)
...
- Trial expiry logic: COVERED ✅
- Upgrade workflow: COVERED ✅
- Worker enforcement: COVERED ✅
...
- PR #756 - Free → starter_trial migration
