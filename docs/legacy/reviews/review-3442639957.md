...
- **Critical (🔴):** 1 - Jest mock hoisting issue
- **Major (🟠):** 3 - Script error handling, labels format, RLS assertions

### Por Issue

#### 🔴 C1: Jest Mock Hoisting Issue (CRITICAL)
...
- **Nodos:** roast, shield, queue-system (detectados automáticamente)
...
- **Invocar:**
  ...
  - Guardian (para security.test.js - verificación RLS)
- **Receipts:**
...
1. **Critical (C1)** - Fix mock hoisting (bloquea ejecución de tests)
...
```javascript
beforeEach(() => {
  const mockSupabaseClientForMockMode = { ... }; // ❌ Declarado aquí

  jest.mock('../../../src/config/mockMode', () => ({
    mockMode: {
      generateMockSupabaseClient: jest.fn(() => mockSupabaseClientForMockMode) // ❌ Referencia undefined
    }
  }));
});
```
...
```javascript
// ✅ Declarar en module scope
let mockSupabaseClientForMockMode;

beforeEach(() => {
  // ✅ Asignar valor, no declarar
  ...
  jest.mock('../../../src/config/mockMode', () => ({
    mockMode: {
      generateMockSupabaseClient: jest.fn(() => mockSupabaseClientForMockMode) // ✅ Ahora accesible
    }
  }));
});
```
...
```bash
--label "test:unit,complementary-flow"  # ❌ Crea un label combinado
```
...
```bash
--label "test:unit" \
--label "complementary-flow"  # ✅ Dos labels separados
```
...
```javascript
expect(true).toBe(true); // ❌ No verifica nada
```
