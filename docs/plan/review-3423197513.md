# CodeRabbit Review #3423197513 - Plan de Aplicación
...
- **Afecta:** Toda la integración Polar - pagos no pueden confirmarse
- **Patrón:** Mismo error que Stripe webhooks (ya resuelto anteriormente)
...
// Después el resto de routes con JSON parsing
...
- **Root cause:** Claim sin verificación
...
- Language consistency ES→EN (1 file)
...
**Orden:** C1
**Por qué:** Sin esto, ningún webhook funciona - integration rota 100%
...
**Orden:** M1 → M4 → M5
**Por qué:** Docs incorrectas = users stuck, tests mal aislados = false positives
...
3. **M4 - Fix test env var cleanup** (10 min)
   - Save original POLAR\_\* env vars in `beforeAll`
   - Restore in `afterAll`
...
**Orden:** M2 → M3
**Por qué:** Misleading marketing = legal/trust issues
...
### ✅ Manual Verification (OBLIGATORIO)

- [ ] Follow POLAR-QUICK-START.md from scratch
- [ ] Create checkout with valid price_id → Success
- [ ] Simulate webhook → Signature validates
...
**Prioridad:** 🔴 CRITICAL (C1 bloqueante)
**Responsable:** Lead Orchestrator → Test Engineer, Documentation Agent, Frontend Dev, Backend Dev
