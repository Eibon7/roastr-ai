# Plan de Implementación: Issues #594 y #808 (Polar Payment Integration)

**Created:** 2025-11-11
**Issues:** #594 (Payment Flow con Polar), #808 (Migrar tests Polar)
**Estimated Time:** 8-10 horas
**Priority:** P1 (Critical para monetización)

---

## 🎯 Objetivo

Completar la integración de Polar como proveedor de pagos y asegurar 100% de cobertura en tests de billing.

---

## 📋 Estado Actual (Assessment)

### Issue #594: ✅ 70-80% IMPLEMENTADO
...
### Issue #808: ⚠️ 4/67 tests fallando
...
**Estrategia:** Opción B - Consolidar tests existentes (2-4h) en lugar de migrar todo
...
**Tareas:**
1. Añadir método `setEntitlementsFromPolarPrice(userId, polarPriceId)`:
...
3. Adaptar plan limits según nuevo mapping:
   - `starter_trial` (Free) → 0 persona fields, nivel fijo
   - `pro` → Features completos
   - `creator_plus` → Features premium
...
- **Fix:** Añadir mock response para Polar price lookup
...
- **Problema:** Validación no implementada para Polar
- **Fix:** Añadir validación en checkout route

**Test 4: subscription route catch block**
- **Problema:** Error handling no cubierto
- **Fix:** Añadir test case específico
...
### FASE 4: Consolidación de Tests (#808)
...
3. Añadir tests faltantes si coverage <90%
4. Documentar qué archivo de tests cubre qué funcionalidad

**Target:** ≥90% cobertura en todos los archivos de billing/Polar

---

### FASE 5: Documentación (#594)

**Archivos a actualizar:**
1. **`.env.example`** - Añadir variables Polar completas:
   ```bash
   # Polar Integration
...
2. **`docs/flows/payment-polar.md`** - Actualizar con:
   ...
   - Código completo de webhook handlers
...
3. **`docs/nodes/billing.md`** - Actualizar:
   ...
   - Añadir "Agentes Relevantes": BackendDev, TestEngineer, Guardian

4. **`docs/issues/issue-payment-polar.md`** - Marcar items completados

5. **`CLAUDE.md`** - Añadir sección Polar:
   ```markdown
...
- `src/services/entitlementsService.js` (añadir Polar support)
- `tests/unit/routes/billing-coverage-issue502.test.js` (arreglar 4 tests)
- `.env.example` (añadir variables Polar)
- `docs/flows/payment-polar.md` (actualizar estado)
- `docs/nodes/billing.md` (añadir Polar, actualizar Agentes Relevantes)
...
- [ ] Cobertura ≥90% en billing/checkout/polarWebhook
- [ ] Documentación de cobertura actualizada
...
2. **FASE 3:** Arreglar tests fallando (1-2h) ← Prioridad para quick win
3. **FASE 4:** Consolidar cobertura (1h)
4. **FASE 2:** EntitlementsService (1-2h)
5. **FASE 5:** Documentación (1h)
6. **FASE 6:** Validación completa (30 min)
