...
- **NITS:** Logger pattern approved ✅, Guardian CLI tests approved ✅
...
beforeAll(() => {
  ...
  // ❌ No cleanup - env vars pollute other test suites
});
```
...
it('should handle text exceeding max length', async () => {
  ...
  expect([200, 400, 500]).toContain(response.status); // ❌ Accepts success!
...
it('should reject text exceeding max length', async () => {
  ...
  // ✅ Assert specific contract - only 400 or 500 acceptable
  ...
  // ✅ Ensure specific validation message
...
it('should require authentication for preview endpoint', async () => {
  ...
  expect([401, 500]).toContain(response.status); // ❌ Accepts server error!
});
...
it('should require authentication for preview endpoint', async () => {
  ...
  // ✅ Only 401 is acceptable for auth failure
  expect(response.status).toBe(401);

  // ✅ Verify error payload contains authorization language
  expect(response.body).toMatchObject({
    success: false,
...
### ✅ NITS (Approved) - 2 items

#### NITS-1: Logger Import Standardization ✅
...
**Order:** M1 → M2 → M3 → Test Run → Verify all tests passing
...
2. **Fix M2 - Ineffective Error Handling Test**
   - Update assertion: `[200, 400, 500]` → `[400, 500]`
...
3. **Fix M3 - Weak Authentication Assertions**
   - Update both auth tests: `[401, 500]` → `401`
...
**Order:** N1 → N2 → N3 → Lint verification
...
- [ ] **Tests Passing:** `npm test -- roast.test.js` → 10/10 ✅
- [ ] **Linting Passing:** `markdownlint docs/` → 0 errors
...
### Quality Metrics:
- **Test Pass Rate:** 5/10 → 10/10 (100%)
- **Test Quality:** Weak assertions → Strict contract enforcement
- **Documentation:** Linting errors → 0 errors
- **Clarity:** Stale WIP → Accurate completion report
...
- tests/integration/roast.test.js:
  ...
  * Strengthened error test: [200,400,500] → [400,500] + error structure validation
  * Hardened auth tests: [401,500] → 401 + payload validation
...
### Testing
- tests/integration/roast.test.js: 5/10 → 10/10 passing (100% pass rate)
...
**Add to coderabbit-lessons.md:** Yes, if this pattern appears ≥2 times
