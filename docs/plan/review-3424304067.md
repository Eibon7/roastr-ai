# CodeRabbit Review #3424304067 - Plan de Aplicación
...
**Fecha:** 2025-11-05
**Política:** 0 CodeRabbit comments antes de merge

---

## 1. Análisis por Severidad

### 🔴 CRITICAL - Security (PII Exposure) - 3 issues

**C1: PII en logs - checkout.js**
- **File:** `src/routes/checkout.js:69-79, :94`
- **Issue:** Logging raw customer_email (PII) to logs
- **Risk:** GDPR violation, data leak en logs de producción
...
- **Fix:** Usar helper de sanitización consistente
...
- **Risk:** PII visible en DevTools, leak en producción
- **Fix:** Remover o guardar con flag NODE_ENV !== 'production'

### 🟠 MAJOR - Data Integrity - 2 issues

**M1: Precio incorrecto - Pro Plan**
- **File:** `frontend/src/components/PolarPricingExample.jsx:45`
- **Issue:** Muestra €15, docs dicen €12/month
- **Risk:** User confusión, expectativa incorrecta
- **Fix:** Actualizar a €12 según docs/POLAR-CONFIGURED.md:12

**M2: Precio incorrecto - Plus Plan**
- **File:** `frontend/src/components/PolarPricingExample.jsx:61`
- **Issue:** Muestra €50, docs dicen €24/month
- **Risk:** Descuento pricing, revenue loss
- **Fix:** Actualizar a €24 según docs/POLAR-CONFIGURED.md:13

### 🟡 MINOR - Documentation - 2 issues

**Min1: Inconsistencia PR status**
- **File:** `docs/POST-MERGE-GDD-WORKFLOW.md:43`
- **Issue:** PR #695 marked as "Merged ✅" pero status es "open"
...
**Skill GDD:** Cargar contexto automáticamente
...
### Security Audit Agent → C1, C2, C3
- **Task:** Buscar patrón PII en logs en TODA la codebase
- **Output:** Helper centralizado para sanitización
...
**Orden:** C1 → C2 → C3 (misma categoría, agrupar)
...
1. Crear `src/utils/piiSanitizer.js`:
   ...
   function sanitizePII(data) {
     if (data.customer_email) {
       data.customer_email_hash = hashEmail(data.customer_email);
       data.customer_email = maskEmail(data.customer_email);
     }
     if (data.email) {
       data.email_hash = hashEmail(data.email);
...
1. Actualizar `PolarPricingExample.jsx:45`: €15 → €12
2. Actualizar `PolarPricingExample.jsx:61`: €50 → €24
...
- `tests/unit/utils/piiSanitizer.test.js` (NEW)
  - ✓ hashEmail genera hash consistente
  - ✓ maskEmail oculta local part
  - ✓ sanitizePII procesa objetos correctamente
  - ✓ No crash con email inválido

- `tests/unit/routes/checkout.security.test.js` (MODIFY)
  - ✓ Logs no contienen email raw
  - ✓ Logs contienen email_hash
  - ✓ customer_email está masked

- `tests/unit/routes/polarWebhook.security.test.js` (MODIFY)
  - ✓ Error logs no exponen PII
  - ✓ Success logs sanitizados

- `tests/unit/frontend/PolarPricingExample.test.js` (NEW)
  - ✓ Pro plan muestra €12
  - ✓ Plus plan muestra €24
  - ✓ No console.log en producción

### Integration Tests:
- Verificar full flow checkout→webhook no rompe con sanitización

### Visual Tests:
- Screenshot de pricing component con precios correctos

### Coverage Goal:
- Mantener ≥90% en archivos modificados
- piiSanitizer.js: 100% coverage (crítico para security)
...
- [x] 0 PII en logs de producción
- [x] Precios UI match documentación
- [x] Docs consistency restaurada

### ✅ Testing
- [x] All tests pass (0 failures)
- [x] Coverage mantiene ≥90%
...
### ✅ Code Quality
- [x] Solución arquitectural (helper centralizado, NO parches)
...
- Coverage: piiSanitizer 100%, checkout 92%→94%, webhook 89%→91%
...
### Issues Addressed
- [MAJOR] M1: Pro plan price €15 → €12 (PolarPricingExample.jsx:45)
- [MAJOR] M2: Plus plan price €50 → €24 (PolarPricingExample.jsx:61)
...
- Coverage: PolarPricingExample 0%→85%
...
### Patrón detectado: PII en logs
- **Problema:** Múltiples lugares logueando PII directamente
- **Solución:** Helper centralizado de sanitización
- **Prevención:** Linter rule para detectar `logger.*email`
- **Agregar a:** `docs/patterns/coderabbit-lessons.md`

### Patrón detectado: Precios hardcoded
- **Problema:** Precios en código != docs
- **Solución:** Test que valida consistency
- **Prevención:** CI check que compara precios
- **Agregar a:** `docs/patterns/coderabbit-lessons.md`
