# CodeRabbit Review #3421415462 - Plan de Aplicación
...
- **Impacto:** Build fallará en producción; componentes frontend no compilarán
- **Afecta también:**
...
- **Patrón:** Primera vez que usamos styled-jsx en el proyecto
...
- **Impacto:** Webhook handler crashea con signatures inválidas (DoS vector)
- **Root cause:** `crypto.timingSafeEqual()` requiere buffers de misma longitud
- **Patrón:** Timing-attack protection mal implementada
...
- **Root cause:** No validación server-side de price_ids
- **Patrón:** Trust user input sin validación (OWASP Top 10 - Broken Access Control)
...
- `test-polar-webhook.sh`: No security warning sobre exposición pública
...
### Test Engineer (ALL)
- Tests de explotación para M1: enviar price_ids inválidos
- Tests para C2: signatures con longitudes diferentes
- Tests para C1: verificar que build pasa después del refactor

### Frontend Dev (C1)
- Refactorizar styled-jsx → Tailwind (ya disponible)
...
```
src/routes/checkout.js          [M1] - Añadir price_id allowlist
...
**Orden:** C2 → M1 → C1
**Por qué:** Seguridad primero, luego build

1. **C2 - Webhook signature validation** (10 min)
   - Añadir signature length check
   - Añadir missing signature check
   - Test: enviar signature vacía, signature de longitud incorrecta
...
3. **C1 - styled-jsx → Tailwind migration** (30 min)
   - CheckoutButton: Convertir `<style jsx>` a clases Tailwind
   - PolarPricingExample: Ídem
   - CheckoutSuccess: Ídem
...
9. Renombrar rama: `feat/polar-integration-complete` → `feature/issue-724`
10. Force push
...
- Nuevos archivos: 100% coverage (son críticos de seguridad)
...
### ✅ Blockers resueltos (OBLIGATORIO)
- [ ] C2: Webhook no crashea con signatures inválidas
- [ ] M1: Price IDs validados server-side
- [ ] C1: Frontend build pasa sin styled-jsx

### ✅ Tests (OBLIGATORIO)
- [ ] 8 tests nuevos añadidos (4 checkout + 4 webhook)
...
**Mitigación:**
- Permitir en dev si POLAR_WEBHOOK_SECRET vacío
- Logging explícito
...
- ❌ Cambios en deployment pipeline
...
- Tests failing después de aplicar fix
- Build failing
...
1. **Styled-jsx sin configuración en CRA**
   - Patrón: Usar `<style jsx>` sin verificar build support
   - Fix: Usar Tailwind (ya disponible) o instalar styled-jsx
   - Prevención: Verificar `npm run build` antes de commit

2. **timingSafeEqual sin length check**
   - Patrón: Asumir buffers de misma longitud
   - Fix: Validar `signature.length === expected.length` antes
   - Prevención: Siempre validar longitud en crypto comparisons

3. **Trust user input sin allowlist**
   - Patrón: Aceptar IDs del cliente sin validar contra lista permitida
   - Fix: Server-side allowlist obligatorio
   - Prevención: NUNCA trust user input para IDs críticos (prices, products, etc.)

**Agregar a:** `docs/patterns/coderabbit-lessons.md` después de merge

---

**Tiempo estimado total:** 90 minutos
**Prioridad:** 🔴 CRITICAL - Bloqueante para merge
**Responsable:** Lead Orchestrator → delega a Security Audit + Test Engineer + Frontend Dev
