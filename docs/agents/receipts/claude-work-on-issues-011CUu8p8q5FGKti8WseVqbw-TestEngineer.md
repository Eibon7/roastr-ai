...
Fixed failing billing and plan limits tests by configuring fail-closed behavior to use 'starter_trial' (most restrictive plan) instead of non-existent 'free' plan, and enabling proper test mode behavior. All core billing tests now pass without requiring real Supabase credentials.
...
### 1. Fail-Closed Using Non-Existent 'free' Plan
...
**Root Cause:**
`planLimitsService` tried to return 'free' plan defaults when database fails (fail-closed behavior), but the application doesn't have a 'free' plan. The entry plan is 'starter_trial' (30-day free trial of starter features).
...
**Problem:**
Tests expected database failure to return requested plan's defaults (e.g., 'pro' → 1000 roasts), but service defaulted to fail-closed behavior (always return 'starter_trial' plan).

**Expected:** 1000 (pro plan default)
**Received:** 10 (starter_trial default after fix #1)
...
1. ✅ `src/services/planLimitsService.js` - Updated 11 occurrences from 'free' to 'starter_trial'
