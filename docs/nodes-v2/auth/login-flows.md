...
sequenceDiagram
    ...
    U->>F: Solicitar password recovery (email)
    F->>RL: POST /api/v2/auth/password-recovery
    ...
    alt Rate limit excedido
        ...
        PG->>PG: Check feature flags
        Note over PG: auth_enable_password_recovery AND<br/>auth_enable_emails
        
        alt Feature flag OFF
            PG-->>F: Fail-closed (AUTH_EMAIL_DISABLED)
            F-->>U: "Password recovery no disponible"
        else Feature flags ON
            PG->>AS: Forward to AuthService
            AS->>AS: Check email infrastructure enabled
            alt Email infra disabled
                AS-->>F: Fail-closed (AUTH_EMAIL_SEND_FAILED)
                F-->>U: "Error al enviar email"
            else Email infra OK
                AS->>SB: admin.listUsers (buscar email)
                
                alt Email no existe
                    AS-->>F: 200 OK (anti-enumeration)
                    F-->>U: "Si el email existe, recibirás un link"
                else Email existe
                    AS->>AS: Verificar role != admin/superadmin
                    ...
                    alt Role = admin o superadmin
                        ...
                        AS->>SB: resetPasswordForEmail(email, redirectUrl)
                        SB->>Email: Enviar email de recovery
                        ...
                        Note over U: Usuario hace clic en link
                        U->>F: Redirect desde email (con token)
                        F->>SB: Verificar token de reset
                        SB-->>F: { session }
                        F->>F: Store tokens
                        F-->>U: Redirect to password update page
                    end
                end
            end
        end
...
**Flags requeridos:**
- `auth_enable_password_recovery` (endpoint gate)
- `auth_enable_emails` (infra de emails)

**Comportamiento si OFF:**
- `auth_enable_password_recovery` OFF → Fail-closed (AUTH_EMAIL_DISABLED)
- `auth_enable_emails` OFF → Fail-closed (AUTH_EMAIL_SEND_FAILED)

**⚠️ Fail-Closed:** Si la infraestructura de email está deshabilitada, el endpoint DEBE fallar (no simular éxito), incluso si el email no existe. Esto previene que usuarios asuman que el sistema está funcionando cuando en realidad no puede enviar emails.
...
**Uso:** El link de reset en el email redirige a esta URL con el token de reset como query parameter.

### Observability

**Eventos logged:**
- `auth_password_recovery_request` (success/failure)
- `auth_email_requested` (antes de enviar, flow: recovery)
- `auth_email_sent` (después de enviar, flow: recovery)
- `auth_email_failed` (si falla el envío)
- `auth_email_blocked` (si infra está deshabilitada)
- `auth_duration_seconds` (histogram)
...
- `auth_email_blocked` (flow: password_recovery, con reason)

### Security

**Anti-Enumeration:**
- Respuesta homogénea independientemente de si el email existe
- Mismo mensaje para email no existente, admin/superadmin, o usuario válido
- No se revela información sobre la existencia de cuentas

**Fail-Closed:**
- Si la infraestructura de email está deshabilitada, el endpoint falla
- No se simula éxito para prevenir confusión
- Logs incluyen razón del bloqueo (sin PII)

**Rate Limiting:**
- Protección contra brute force y abuse
- Bloqueo progresivo para prevenir ataques coordinados

**Token Security:**
- Tokens de reset tienen expiración (configurado en Supabase)
- Tokens son de un solo uso
- Tokens se invalidan después de uso exitoso

### Implementación

**Archivos principales:**
- **Route:** `apps/backend-v2/src/routes/auth.ts:389-455`
- **Service:** `apps/backend-v2/src/services/authService.ts:675-778`
- **Email Service:** `apps/backend-v2/src/services/authEmailService.ts:163-238`

**Tests:**
- `apps/backend-v2/tests/unit/services/authService-passwordRecovery.privacy.test.ts` (privacy/anti-enumeration)
- `apps/backend-v2/tests/unit/services/authEmailService.test.ts` (email service)
- `apps/backend-v2/tests/flow/auth-http.endpoints.test.ts` (HTTP endpoints)

**Script de prueba manual:**
- `apps/backend-v2/scripts/test-password-recovery.ts`
