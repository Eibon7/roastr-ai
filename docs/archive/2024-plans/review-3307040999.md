...
**Phase 2: Code Quality (Priority: P1)** 4. m5: Replace `any` with `unknown` in types

**Phase 3: Documentation (Priority: P2)** 5. m2: Fix markdown linting in README
...
```javascript
const avgRQCScore =
  metrics.length > 0 ? metrics.reduce((sum, m) => sum + (m.rqc_score || 0), 0) / metrics.length : 0;
