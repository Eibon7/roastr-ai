...
#### 🟠 Major #1: Fail-closed test doesn't simulate actual database outage
...
> **Fail-closed path never simulates a Supabase outage** — The test uses `mockResolvedValue({ data: null, error: { message: 'Not found' } })`, which simulates "flag not found" but not an actual DB connection failure. For a true fail-closed scenario, use:
>
> ```javascript
...
1. **Current test:** `mockResolvedValue({ data: null, error: {...} })` → Query succeeds, flag not found → handleMissingFlag → returns default `is_enabled: false` → AUTOPOST_DISABLED
2. **Expected test:** `mockRejectedValue(new Error(...))` → Query fails (DB outage) → catch block → fail-closed behavior → KILL_SWITCH_ERROR

The test MUST validate scenario #2 to ensure fail-closed works during actual database outages.

---

#### 🟢 Minor #2: Bare URL in planning document
...
**Status:** ✅ **ALREADY FIXED** (Commit 22d48d7b)
...
#### 🟢 Minor #3: Missing language tag in SUMMARY.md
...
- ✅ Tests will be more accurate (proper exception testing)
- ⚠️ Test may fail after fix if middleware lacks error handling
- ⚠️ May require middleware implementation before test passes
- ✅ More robust fail-closed validation

---

## FASE 3: Risk Assessment

### Major Risk (Issue #1) ⚠️⚠️⚠️
...
### Option 1: Update Test + Add Middleware Error Handling (RECOMMENDED) ✅
...
### Option 2: Update Test Only (CONDITIONAL) ⚠️
...
### Option 3: Leave Test As-Is (REJECTED) ❌
...
1. ✅ Test validates actual DB outage scenario
2. ✅ Test uses mockRejectedValue (proper exception testing)
3. ✅ Test may pass immediately (if error handling works)

**IF middleware lacks error handling:**

1. ⚠️ Test exposes missing error handling (GOOD - catches bug)
2. ✅ Middleware enhanced with try/catch + fail-closed
3. ✅ Production becomes more robust
4. ✅ Test validates new error handling

### Long-term Outcomes

1. ✅ Integration tests accurately validate exception scenarios
2. ✅ Middleware handles database outages gracefully
3. ✅ Production fail-closed behavior verified
4. ✅ No crashes on DB connection failures
...
- Test fix: mockResolvedValue → mockRejectedValue
...
### Issues Addressed
- 🟠 Major: Fail-closed test doesn't simulate DB outage (killSwitch-issue-414.test.js:421-436)
- 🟢 Minor: Bare URL (ALREADY FIXED in 22d48d7b)
- 🟢 Minor: Language tag (ALREADY FIXED in 79a941d3)
...
- tests/integration/killSwitch-issue-414.test.js (+6/-6 lines)
  - AC5 test 3: mockResolvedValue → mockRejectedValue
...
- ✅ Test accuracy improved (exception handling verified)
...
### Issues Addressed
- 🟠 Major: Missing fail-closed error handling in middleware
- 🟠 Major: Fail-closed test doesn't simulate DB outage (killSwitch-issue-414.test.js:421-436)
- 🟢 Minor: Bare URL (ALREADY FIXED in 22d48d7b)
- 🟢 Minor: Language tag (ALREADY FIXED in 79a941d3)
...
- tests/integration/killSwitch-issue-414.test.js (+6/-6 lines)
  - AC5 test: mockResolvedValue → mockRejectedValue
...
- 🟠 Major: 1 (test accuracy + potential middleware enhancement)
- 🟢 Minor: 2 (already fixed in previous commits)
