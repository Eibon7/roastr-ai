class IntegrationPanel {
            ...
            <div class="config-section">
                ...
                <div class="trigger-words">
                    ${(config.triggerWords || []).map(word => 
                        `<span class="trigger-word">${word}</span>`
                    ).join('')}
                </div>
            </div>
    ...
    async loadLogs(logType) {
        try {
            ...
            if (response.ok) {
                ...
                document.getElementById('tabContent').innerHTML = '<div class="error">Error cargando logs</div>';
            }
        } catch (error) {
    ...
    async renderLogFiles(logType, files) {
        ...
        let html = '';
        if (logType === 'shield') {
            html += '<div class="shield-warning show">⚠️ Los siguientes logs pueden contener contenido sensible u ofensivo.</div>';
        }

        for (const file of files.slice(0, 3)) { // Show only first 3 files
            try {
