...
- contexto: "Database query fails with invalid parameter"
  trace: | 1. Error: Invalid parameter in query 2. Immediate: Database query with NULL value 3. Called by: ShieldService.executeAction() 4. Called by: AnalyzeToxicityWorker.process() 5. Called by: Worker.run() 6. Original trigger: Worker receives incomplete context from API 7. Fix: Added validation in context parsing + default values
  principle:
...
- "./find-polluter.sh '.git' 'src/\*_/_.test.ts'"
