...
describe('Cost control - quota enforcement', () => {
  ...
  it('should enforce Free tier quota (100 roasts)', async () => {
    ...
    await expect(generateRoast({ orgId: org.id, comment: 'Over limit' })).rejects.toMatchObject({
      code: 'QUOTA_EXCEEDED',
  ...
  it('should allow usage exactly at limit', async () => {
    ...
    // 101st should fail
    await expect(generateRoast({ orgId: org.id, comment: 'Over limit' })).rejects.toThrow(
      'QUOTA_EXCEEDED'
    );
  });
  ...
  it('should prevent negative usage manipulation', async () => {
    ...
    await expect(setUsage(org.id, -10)).rejects.toThrow('INVALID_USAGE');
...
describe('Cost control - tier upgrades', () => {
  it('should apply new quota immediately on upgrade', async () => {
    ...
    await expect(generateRoast({ orgId: org.id, comment: 'Should fail' })).rejects.toThrow(
      'QUOTA_EXCEEDED'
    );
...
describe('Cost control - edge cases', () => {
  it('should handle concurrent requests at quota boundary', async () => {
    ...
    const promises = Array(10)
      .fill(null)
      .map(() => generateRoast({ orgId: org.id, comment: 'Concurrent test' }).catch((err) => err));
  ...
  it('should use database transactions for usage increment', async () => {
    ...
    await expect(async () => {
      ...
      try {
        ...
        // Increment usage
        await client.query('UPDATE organizations SET roasts_used = roasts_used + 1 WHERE id = $1', [
          org.id
        ]);
