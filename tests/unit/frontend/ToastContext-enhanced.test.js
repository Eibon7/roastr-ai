...
const TestComponent = () => {
  ...
  return (
    <div>
      <button onClick={() => toast.show('Basic toast')} data-testid="basic-toast">
        Basic Toast
      </button>
      <button
      ...
      <button onClick={() => toast.success('Success message')} data-testid="success-toast">
        Success Toast
      </button>
      <button
      ...
      <button
        onClick={() =>
          toast.action('Action toast', 'Click me', () => console.log('Action clicked'), {
            type: 'warning'
          })
        }
...
describe('Enhanced ToastContext', () => {
  ...
  describe('Security Features', () => {
    test('should sanitize XSS content', async () => {
      ...
      expect(
        screen.getByText(/&lt;script&gt;alert\(&quot;xss&quot;\)&lt;\/script&gt;Malicious content/)
      ).toBeInTheDocument();
    ...
    test('should handle various XSS attack vectors', async () => {
      ...
      maliciousInputs.forEach((input) => {
        ...
        expect(
          screen.getByText(new RegExp(sanitized.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Error Handling', () => {
    test('should handle action callback errors gracefully', async () => {
      ...
      act(() => {
        result.current.toast.action('Error toast', 'Error action', () => {
