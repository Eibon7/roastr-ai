...
describe('PasswordHistoryService', () => {
  ...
  describe('isPasswordRecentlyUsed', () => {
    it('should return false when password history is disabled', async () => {
      ...
      const result = await passwordHistoryService.isPasswordRecentlyUsed(
        'user-123',
        'testPassword123'
      );
    ...
    it('should return true when password matches recent history', async () => {
      ...
      mockSupabaseServiceClient.select.mockResolvedValue({
        data: [{ password_hash: 'matching_hash' }, { password_hash: 'different_hash' }],
        error: null
      });
    ...
    it('should return false when password does not match any in history', async () => {
      ...
      mockSupabaseServiceClient.select.mockResolvedValue({
        data: [{ password_hash: 'old_hash_1' }, { password_hash: 'old_hash_2' }],
        error: null
      });
    ...
    it('should handle database errors gracefully', async () => {
      ...
      expect(result).toBe(false);
      expect(logger.error).toHaveBeenCalledWith('Error checking password history:', {
  ...
  describe('addToPasswordHistory', () => {
    it('should return true when password history is disabled', async () => {
      ...
      const result = await passwordHistoryService.addToPasswordHistory(
        'user-123',
        'testPassword123'
      );
    ...
    it('should handle database insertion errors', async () => {
      ...
      expect(result).toBe(false);
      expect(logger.error).toHaveBeenCalledWith('Error adding password to history:', {
  ...
  describe('cleanupOldPasswords', () => {
    ...
    it('should handle cleanup errors gracefully', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error fetching passwords for cleanup:', {
        message: 'Fetch error'
      });
    });
  });
  ...
  describe('clearPasswordHistory', () => {
    it('should successfully clear all password history', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('Cleared password history for user', {
        userId: 'user-123'
      });
    });
    ...
    it('should handle deletion errors', async () => {
      ...
      expect(result).toBe(false);
      expect(logger.error).toHaveBeenCalledWith('Error clearing password history:', {
  ...
  describe('getPasswordHistoryStats', () => {
    ...
    it('should handle database errors gracefully', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error getting password history stats:', {
        message: 'Database error'
      });
    });
  });
