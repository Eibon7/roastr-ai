...
describe('StyleProfileService Security Fixes', () => {
  ...
  describe('AAD Implementation', () => {
    ...
    it('should require userId and platform for encryption', async () => {
      ...
      await expect(service.encryptStyleProfile(styleProfile, null, 'twitter')).rejects.toThrow(
        'userId and platform are required for secure encryption'
      );

      await expect(service.encryptStyleProfile(styleProfile, 'user123', null)).rejects.toThrow(
        'userId and platform are required for secure encryption'
      );
    });
    ...
    it('should require userId and platform for decryption', async () => {
      ...
      await expect(service.decryptStyleProfile(encryptedData, null, 'twitter')).rejects.toThrow(
        'userId and platform are required for secure decryption'
      );

      await expect(service.decryptStyleProfile(encryptedData, 'user123', null)).rejects.toThrow(
        'userId and platform are required for secure decryption'
      );
    });
    ...
    it('should fail decryption with wrong userId/platform (AAD mismatch)', async () => {
      ...
      await expect(service.decryptStyleProfile(encrypted, 'wrong-user', platform)).rejects.toThrow(
        'Failed to decrypt style profile: data may be corrupted or tampered with'
      );

      // Try to decrypt with wrong platform
      await expect(
        service.decryptStyleProfile(encrypted, userId, 'wrong-platform')
  ...
  describe('Input Validation', () => {
    ...
    it('should validate encrypted data structure for decryption', async () => {
      ...
      await expect(
        service.decryptStyleProfile(incompleteData, 'user123', 'twitter')
    ...
    it('should validate encrypted profile structure for storage', async () => {
      ...
      await expect(
        service.storeStyleProfile('user123', 'twitter', incompleteProfile)
