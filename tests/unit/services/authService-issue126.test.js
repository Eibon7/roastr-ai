...
describe('AuthService.updateUserPlan - Issue #126 Improvements', () => {
  ...
  describe('Standardized Error Handling', () => {
    ...
    it('should fail fast on user update errors', async () => {
      ...
      await expect(authService.updateUserPlan('invalid-user', 'pro', 'admin-123')).rejects.toThrow(
        'Failed to update user plan: User not found'
      );
    });
  });
  ...
  describe('Rollback Mechanism', () => {
    it('should rollback plan change when applyPlanLimits fails', async () => {
      ...
      const rollbackSpy = jest.spyOn(authService, 'rollbackPlanChange').mockResolvedValue(true);

      await expect(authService.updateUserPlan('user-123', 'pro', 'admin-123')).rejects.toThrow(
