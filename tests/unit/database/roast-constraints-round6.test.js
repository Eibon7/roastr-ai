...
describe('Database Constraints Round 6', () => {
  ...
  describe('Status Constraint Validation', () => {
    test('should enforce valid status values', async () => {
      ...
      for (const status of validStatuses) {
        ...
        const result = await mockSupabase.from('roasts_metadata').insert({ status }).single();
  ...
  describe('Platform Constraint Validation', () => {
    test('should enforce valid platform values', async () => {
      ...
      for (const platform of validPlatforms) {
        ...
        const result = await mockSupabase.from('roasts_metadata').insert({ platform }).single();
  ...
  describe('RLS Policy Enhancements', () => {
    test('should enforce user isolation with USING clause', async () => {
      ...
      let result = await mockSupabase.from('roasts_metadata').select('*').eq('user_id', userId);
      ...
      result = await mockSupabase.from('roasts_metadata').select('*').eq('user_id', otherUserId);
    ...
    test('should enforce user isolation with WITH CHECK clause', async () => {
      ...
      let result = await mockSupabase.from('roasts_metadata').insert({ user_id: userId }).single();
      ...
      result = await mockSupabase.from('roasts_metadata').insert({ user_id: otherUserId }).single();
    ...
    test('should allow service role full access with WITH CHECK', async () => {
      ...
      const selectResult = await serviceRoleMock.from('roasts_metadata').select('*');
  ...
  describe('Language Constraint Validation', () => {
    test('should enforce language constraint', async () => {
      ...
      for (const language of validLanguages) {
        ...
        const result = await mockSupabase.from('roasts_metadata').insert({ language }).single();
