...
describe('Admin Plan Upgrade/Downgrade API', () => {
  ...
  describe('POST /api/auth/admin/users/update-plan', () => {
    it('should successfully upgrade user plan from free to pro', async () => {
      ...
      const response = await request(app).post('/api/auth/admin/users/update-plan').send({
        userId: 'user-123',
        newPlan: 'pro'
      });
      ...
      expect(mockAuthService.updateUserPlan).toHaveBeenCalledWith(
        'user-123',
        'pro',
        'admin-id-123'
      );
    ...
    it('should reject plan change when validation fails', async () => {
      ...
      mockAuthService.updateUserPlan.mockRejectedValue(
        new Error(
          'Plan change not allowed: Usage exceeds free plan limits. Warnings: You have 150 roasts this month, free plan allows only 10'
        )
      );
      ...
      expect(response.body.error).toContain(
        'Plan change not allowed: Usage exceeds free plan limits'
      );
    ...
    it('should handle database update errors gracefully', async () => {
      ...
      mockAuthService.updateUserPlan.mockRejectedValue(
        new Error('Failed to update user plan: Database connection failed')
      );
    ...
    it('should verify admin ID is passed to service', async () => {
      ...
      await request(app).post('/api/auth/admin/users/update-plan').send({
        userId: 'user-123',
        newPlan: 'pro'
      });
