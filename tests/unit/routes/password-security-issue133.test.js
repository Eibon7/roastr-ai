...
describe('Password Security Enhancements (Issue #133)', () => {
    ...
    it('should validate password meets all requirements', () => {
      ...
      const validatePassword = (password) => {
        ...
        const strongCriteria = [
          criteria.uppercase || criteria.special,
          criteria.lowercase,
          criteria.number
        ];
        ...
        if (strongCriteriaPassed < 2) {
          return {
            valid: false,
            message: 'Password must contain lowercase, numbers, and uppercase or special characters'
          };
        }
  ...
  describe('Password History', () => {
    it('should check password history when enabled', async () => {
      ...
      expect(mockPasswordHistoryService.addToPasswordHistory).toHaveBeenCalledWith(
        userId,
        newPassword
      );
    });
