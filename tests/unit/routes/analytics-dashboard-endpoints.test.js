...
describe('Analytics dashboard endpoints', () => {
  ...
  beforeEach(() => {
    ...
    mockAnalyticsDashboardService.getDashboardData.mockReset();
    mockAnalyticsDashboardService.getBillingAnalytics.mockReset();
    mockAnalyticsDashboardService.exportAnalytics.mockReset();
  });
  ...
  it('should handle dashboard errors gracefully', async () => {
    ...
    // Use unique parameters to avoid cache from previous test
    await request(app)
      .get('/api/analytics/dashboard?range=7&group_by=week&platform=twitter')
      .expect(500)
      .expect((res) => {
