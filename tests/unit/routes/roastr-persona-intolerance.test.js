...
// Mock flags before requiring user routes
...
describe('Roastr Persona - Lo que no tolero (Issue #149)', () => {
  ...
  beforeEach(() => {
    ...
    mockCreateUserClient = jest.fn(() => mockSupabaseServiceClient);
  ...
  describe('GET /api/user/roastr-persona', () => {
    it('should return both identity and intolerance data when available', async () => {
      const mockUserData = {
        ...
        lo_que_me_da_igual_encrypted: 'encrypted_cG9sw610aWNh', // "política" encrypted
        lo_que_me_da_igual_visible: false,
        lo_que_me_da_igual_created_at: '2024-01-01T00:00:00Z',
        lo_que_me_da_igual_updated_at: '2024-01-02T00:00:00Z'
      };
    ...
    it('should handle missing intolerance data gracefully', async () => {
      ...
      expect(response.body.data.loQueMeDaIgual).toBe(null);
      expect(response.body.data.hasToleranceContent).toBe(false);
    });
    ...
    it('should handle decryption errors gracefully', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.data.loQueNoTolero).toBe(null);
      expect(response.body.data.hasIntoleranceContent).toBe(false); // Field exists but decryption failed = null
    });
  });
  ...
  describe('POST /api/user/roastr-persona', () => {
    ...
    it('should save intolerance preferences successfully', async () => {
      ...
      expect(response.body.data.loQueNoTolero).toBe('insultos raciales, comentarios sobre peso');
