...
describe('POST /api/user/roastr-persona/validate', () => {
  ...
  describe('Prompt Injection Detection', () => {
    it('should reject content with prompt injection patterns', async () => {
      ...
      expect(response.body.error).toBe(
        'El contenido contiene patrones no permitidos o intenta manipular el sistema'
      );
    });
  ...
  describe('Total Length Validation', () => {
    ...
    it('should reject if total length exceeds 900 characters', async () => {
      ...
      expect(response.body.valid).toBe(false);
      expect(response.body.error).toBe(
        'El total de todos los campos no puede superar los 900 caracteres'
      );
  ...
  describe('Authentication', () => {
    it('should require authentication', async () => {
      ...
      const response = await request(app).post('/api/user/roastr-persona/validate').send({
        field: 'loQueMeDefine',
        value: 'test'
      });
