  ...
  describe('POST /api/roast/preview', () => {
    ...
    it.skip('should validate required text parameter', async () => {
      ...
      expect(response.body).toMatchObject({
        ...
        details: expect.arrayContaining([expect.stringContaining('Text is required')])
      });
    });
    ...
    it.skip('should validate text length', async () => {
      ...
      expect(response.body).toMatchObject({
        success: false,
        error: 'Validation failed',
        details: expect.arrayContaining([expect.stringContaining('less than 2000 characters')])
      });
    });
    ...
    it.skip('should validate tone parameter', async () => {
      ...
      expect(response.body).toMatchObject({
        success: false,
        error: 'Validation failed',
        details: expect.arrayContaining([expect.stringContaining('Tone must be one of')])
      });
    });
    ...
    it.skip('should handle empty text', async () => {
      ...
      expect(response.body).toMatchObject({
        success: false,
        error: 'Validation failed',
        details: expect.arrayContaining([expect.stringContaining('Text cannot be empty')])
      });
    });
  ...
  describe('Authentication', () => {
    it('should require authentication for all endpoints', async () => {
      ...
      for (const endpoint of endpoints) {
        const response = await request(app)[endpoint.method](endpoint.path).send({ text: 'test' });
