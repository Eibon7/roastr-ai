  ...
  describe('GET /api/notifications', () => {
    describe('Type parameter validation', () => {
      it('should accept valid notification types', async () => {
        ...
        for (const type of validTypes) {
          ...
          const response = await request(app).get(`/api/notifications?type=${type}`).expect(200);
      ...
      it('should return 400 for invalid notification type', async () => {
        const response = await request(app).get('/api/notifications?type=invalid_type').expect(400);

        expect(response.body).toEqual({
          success: false,
          error:
            'Invalid notification type. Must be one of: payment_failed, subscription_canceled, upgrade_success, subscription_status, plan_change, plan_change_blocked'
        });
    ...
    describe('Error handling', () => {
      ...
      it('should handle unexpected errors', async () => {
        notificationService.getUserNotifications.mockRejectedValueOnce(
    ...
    describe('Cursor-based pagination', () => {
      ...
      it('should fall back to offset pagination when cursor is not provided', async () => {
        notificationService.getUserNotifications.mockResolvedValueOnce({
          success: true,
          data: [{ id: 'notif-1', created_at: '2024-01-01T12:00:00.000Z' }],
          hasMore: false,
          nextCursor: null
        });

        const response = await request(app).get('/api/notifications?offset=10&limit=1').expect(200);

        expect(response.body).toEqual({
          success: true,
          data: {
            notifications: [{ id: 'notif-1', created_at: '2024-01-01T12:00:00.000Z' }],
            pagination: {
              limit: 1,
              hasMore: false,
              offset: 10
      ...
      it('should return 400 for empty cursor string', async () => {
        const response = await request(app).get('/api/notifications?cursor=').expect(400);
      ...
      it('should combine cursor pagination with filters', async () => {
        ...
        const response = await request(app)
          .get(
            `/api/notifications?cursor=${encodeURIComponent(cursor)}&status=unread&type=payment_failed&include_expired=true`
          )
          .expect(200);
      ...
      it('should set nextCursor=null when hasMore=false even with data', async () => {
        ...
        notificationService.getUserNotifications.mockResolvedValueOnce({
          success: true,
          data: [{ id: 'notif-1', created_at: '2024-01-01T11:00:00.000Z' }],
          hasMore: false,
        ...
        expect(response.body).toEqual({
          success: true,
          data: {
            notifications: [{ id: 'notif-1', created_at: '2024-01-01T11:00:00.000Z' }],
            pagination: {
              limit: 50,
              hasMore: false,
              nextCursor: null
