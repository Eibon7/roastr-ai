...
jest.mock('../../../src/services/dataExportService', () => {
  return jest.fn().mockImplementation(() => ({
    exportUserData: jest.fn().mockImplementation(() =>
      Promise.resolve({
        success: true,
...
describe('User Profile Settings (Issue #258) - Simplified', () => {
  ...
  describe('POST /api/user/data-export (Issue #258)', () => {
    ...
    it('should require authentication', async () => {
      await request(app).post('/api/user/data-export').expect(401);
    });
  ...
  describe('Integration Tests', () => {
    it('should handle multiple requests properly', async () => {
      const promises = [1, 2, 3].map(() =>
        request(app).post('/api/user/data-export').set('Authorization', `Bearer ${authToken}`)
      );
