...
function processResults(results) {
  ...
  Object.keys(spec14Categories).forEach((categoryKey) => {
    ...
    report.spec14.categories[categoryKey] = {
      ...
      missing_scenarios: category.required.filter(
        (req) =>
          !categoryTests.some(
            (test) =>
              test.title.toLowerCase().includes(req.toLowerCase()) ||
  ...
  const requirementsValidation = {
    ...
    contract_tests_complete:
      report.spec14.categories.adapter_contracts?.missing_scenarios.length === 0,
    idempotency_tests_complete:
      report.spec14.categories.idempotency?.missing_scenarios.length === 0,
...
function generateConsoleSummary(report) {
  ...
  console.log(
    `🧪 Tests: ${spec14.summary.passed_tests}/${spec14.summary.total_tests} passed (${spec14.summary.success_rate}%)`
  );
  ...
  Object.keys(spec14.categories).forEach((categoryKey) => {
    ...
    console.log(
      `  ${status} ${category.name}: ${category.passed_tests}/${category.total_tests} (${category.success_rate}%)`
    );
