...
describe('Tier Validation Monitoring Integration - Issue #396', () => {
  ...
  describe('POST /api/monitoring/alerts/test', () => {
    ...
    it('should send test alert successfully', async () => {
      jest
        .spyOn(tierValidationMonitoringService, 'sendAlert')
    ...
    it('should use default values for test alert', async () => {
      ...
      expect(response.body.data.alertDetails.message).toContain(
  ...
  describe('End-to-End Monitoring Workflow', () => {
    it('should demonstrate complete monitoring lifecycle', async () => {
      ...
      let healthResponse = await request(app).get('/api/monitoring/health').expect(200);
      ...
      const metricsResponse = await request(app).get('/api/monitoring/metrics').expect(200);
      ...
      healthResponse = await request(app).get('/api/monitoring/health');
      ...
      await request(app).post('/api/monitoring/cache/clear').expect(200);

      // 8. Verify system is reset (should be healthy since we cleared cache)
      healthResponse = await request(app).get('/api/monitoring/health').expect(200);
      expect(['healthy', 'degraded']).toContain(healthResponse.body.data.status);
    });
  });
});
