  ...
  if (!data._fixtureMetadata) {
    ...
    if (!meta.generatedAt) {
      ...
      if (daysSinceGenerated > MAX_FIXTURE_AGE_DAYS) {
        issues.push(`Fixture is ${Math.round(daysSinceGenerated)} days old (max: ${MAX_FIXTURE_AGE_DAYS})`);
      }
    }
...
function validateDataQuality(data, filename) {
  ...
  if (data.data) {
    ...
    if (data.data.accounts) {
      const testAccounts = data.data.accounts.filter(acc => 
        acc.handle && acc.handle.includes('test') || acc.handle.includes('fixture')
      );
