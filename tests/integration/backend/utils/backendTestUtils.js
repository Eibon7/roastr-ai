  ...
  if (!useBackendFixtures && !enableMockMode) {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error(`Health check timeout after ${fallbackTimeout}ms`)),
          fallbackTimeout
        )
      );
...
const authenticateTestUser = async (testContext) => {
  ...
  if (!email || !password) {
    throw new Error(
      'TEST_USER_EMAIL and TEST_USER_PASSWORD must be set for backend integration tests'
    );
  }
...
const loadFixtureIfNeeded = async (fixtureName, apiFetcher, testContext = null) => {
  const useBackendFixtures =
    process.env.USE_BACKEND_FIXTURES === 'true' || (testContext && testContext.useBackendFixtures);
  const fallbackEnabled = process.env.FALLBACK_TO_FIXTURES_ON_ERROR !== 'false';
