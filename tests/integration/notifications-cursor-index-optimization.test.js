...
describe('Notifications Cursor Pagination Index Optimization (Issue #275)', () => {
  ...
  beforeAll(async () => {
    ...
    await supabaseServiceClient.from('user_notifications').delete().eq('user_id', testUserId);
  });
  ...
  it('should verify the optimal cursor pagination index exists', async () => {
    ...
    const { data: indexes, error } = await supabaseServiceClient.rpc('get_table_indexes', {
    ...
    if (error && error.message?.includes('function')) {
      ...
      const cursorIndex = result.data?.find(
        (idx) => idx.indexname === 'idx_user_notifications_cursor_pagination'
      );
      ...
      const cursorIndex = indexes?.find(
        (idx) => idx.index_name === 'idx_user_notifications_cursor_pagination'
      );
