...
test.describe('Shield UI Visual Tests', () => {
  ...
  for (const viewport of VIEWPORTS) {
    test.describe(`${viewport.name} viewport (${viewport.width}x${viewport.height})`, () => {
      ...
      test('should show filter controls', async ({ page }) => {
        ...
        await expect(page.locator('div:has(select)').first()).toHaveScreenshot(
          `shield-filters-${viewport.name}.png`
        );
      });
      ...
      test('should show revert button for non-reverted actions', async ({ page }) => {
        ...
        const actionWithRevert = page.locator(
          '[data-testid="shield-action"]:has(button:has-text("Revertir"))'
        );
        await expect(actionWithRevert.first()).toHaveScreenshot(
          `shield-revert-button-${viewport.name}.png`
        );
      });
      ...
      test('should show reverted status for reverted actions', async ({ page }) => {
        ...
        const revertedAction = page.locator(
          '[data-testid="shield-action"]:has-text("Revertido el")'
        );
        await expect(revertedAction.first()).toHaveScreenshot(
          `shield-reverted-status-${viewport.name}.png`
        );
      });
    });
  }
  ...
  test.describe('Interactive States', () => {
    ...
    test('should show revert confirmation dialog', async ({ page }) => {
      ...
      await expect(
        page.locator('text=¿Estás seguro de que quieres revertir esta acción de Shield?')
      ).toBeVisible();
    ...
    test('should show loading state with controlled timing', async ({ page }) => {
      ...
      try {
        await page.waitForSelector(
          '[data-testid="loading-indicator"], .animate-pulse, .loading, .spinner',
          {
            timeout: 8000
          }
        );
        ...
        const loadingIndicator = page
          ...
          .or(page.locator('.animate-pulse, .loading, .spinner'));
    ...
    test('should show feature flag disabled state', async ({ page }) => {
      ...
      await expect(
        page.locator('text=La interfaz de Shield está actualmente en desarrollo')
      ).toBeVisible();
  ...
  test.describe('Edge Cases - Enhanced Resilience (CodeRabbit Round 4)', () => {
    ...
    test('should gracefully handle network timeouts and retries', async ({ page }) => {
      ...
      await expect(
        page
          .locator('[data-testid="shield-event"]:has-text("This is offensive content")')
    ...
    test('should handle selector variations and fallbacks', async ({ page }) => {
      ...
      const mainContainer = page
        .locator('[data-testid="shield-container"]')
      ...
      const filterContainer = page
        .locator('[data-testid="shield-filters"]')
      ...
      const eventsList = page
        .locator('[data-testid="shield-events-list"]')
  ...
  test.describe('Accessibility Visual Checks', () => {
    test('should show proper focus states', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="refresh-button"], button:has-text("Actualizar")', {
        timeout: 10000
      });
      const refreshButton = page
        .locator('[data-testid="refresh-button"]')
        .first()
        .or(page.locator('button:has-text("Actualizar")').first());
