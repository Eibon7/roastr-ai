...
test.describe('Shield UI Visual Stability - Round 3 Validation', () => {
  ...
  test.describe('Selector Resilience Validation (Round 3)', () => {
    test('should handle missing data-testid attributes gracefully', async ({ page }) => {
      ...
      const shieldIcon = page.locator('[data-testid="shield-icon"]').or(
        page.locator('[aria-label*="Shield"]')
      );

      const shieldEvent = page.locator('[data-testid="shield-event"]').or(
        page.locator('text=This is offensive content')
      );

      const revertButton = page.locator('[data-testid="revert-button"]').or(
        page.locator('button:has-text("Revertir")')
      );
      ...
      const eventContainer = page.locator('[data-testid="shield-events-container"]').or(
        page.locator('.shield-events').or(
          page.locator('div:has(text="This is offensive content")')
        )
      );
  ...
  test.describe('Network Stability Validation (Round 3)', () => {
    ...
    test('should handle slow network responses', async ({ page }) => {
      ...
      await page.waitForSelector(
        '[data-testid="shield-event"], text=This is offensive content', 
        { timeout: 15000 }
      );

      const eventContent = page.locator('[data-testid="shield-event"]').or(
        page.locator('text=This is offensive content')
      );
  ...
  test.describe('Animation and Timing Stability (Round 3)', () => {
    test('should have consistent timing for interactive elements', async ({ page }) => {
      ...
      const dialog = page.locator('[data-testid="revert-dialog"]').or(
        page.locator('[role="dialog"]').or(
          page.locator('text=Confirmar reversión')
        )
      );
    ...
    test('should handle focus states consistently', async ({ page }) => {
      ...
      const focusableElements = [
        page.locator('[data-testid="refresh-button"]').or(
          page.locator('button:has-text("Actualizar")')
        ),
        page.locator('[data-testid="time-filter"]').or(
          page.locator('select').first()
        ),
        page.locator('[data-testid="category-filter"]').or(
  ...
  test.describe('Data Consistency Validation (Round 3)', () => {
    test('should display consistent timestamp formats', async ({ page }) => {
      ...
      const timestamps = page.locator('[data-testid="event-timestamp"]').or(
        page.locator('.timestamp').or(
          page.locator('time')
        )
      );
      ...
      if (timestampCount > 0) {
        for (let i = 0; i < timestampCount; i++) {
          ...
          expect(timestampText).toMatch(/\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2}|hace \d+ (minuto|hora|día)s?/);
        }
      }
    });
    ...
    test('should handle empty states consistently', async ({ page }) => {
      ...
      const emptyState = page.locator('[data-testid="empty-state"]').or(
        page.locator('text=No hay contenido interceptado').or(
      ...
      const emptyTitle = page.locator('[data-testid="empty-title"]').or(
        page.locator('text=No hay contenido interceptado')
      );

      const emptyDescription = page.locator('[data-testid="empty-description"]').or(
        page.locator('text=Aún no se ha interceptado ningún contenido')
      );
  ...
  test.describe('Filter Interaction Stability (Round 3)', () => {
    test('should handle filter changes without UI flicker', async ({ page }) => {
      ...
      const timeFilter = page.locator('[data-testid="time-filter"]').or(
        page.locator('select').first()
      );
      ...
      const content = page.locator('[data-testid="shield-events-container"]').or(
        page.locator('.shield-events').or(
          page.locator('main')
        )
      );
    ...
    test('should recover from API errors gracefully', async ({ page }) => {
      ...
      if (await errorMessage.first().isVisible()) {
        ...
        const retryButton = page.locator('[data-testid="retry-button"]').or(
          page.locator('button:has-text("Reintentar")').or(
            page.locator('button:has-text("Actualizar")')
          )
        );
        ...
        if (await retryButton.first().isVisible()) {
          ...
          await expect(
            page.locator('[data-testid="shield-event"]').or(
              page.locator('text=This is offensive content')
            ).first()
