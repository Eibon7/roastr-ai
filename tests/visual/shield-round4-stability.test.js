...
describe('Shield UI Visual Tests - Round 4 Stability', () => {
  ...
  test.describe('Enhanced Timezone and Locale Handling', () => {
    test('should display consistent timestamps across environments', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="timestamp"], .timestamp, time, [datetime]', {
        timeout: 10000
      });
    ...
    test('should handle locale-specific formatting consistently', async ({ page }) => {
      ...
      const numberElements = await page
        .locator('[data-testid="count"], .count, .total, text=/\\d+/')
        .all();
  ...
  test.describe('Network Resilience and Error Handling', () => {
    test('should handle network timeouts gracefully', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="shield-container"], .shield-main, main', {
        timeout: 20000
      });

      // Verify content loads after network recovery
      const mainContent = page
        .locator('[data-testid="shield-container"], .shield-main, main')
        .first();
    ...
    test('should handle intermittent connectivity issues', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="shield-title"], h1, .page-title', {
        timeout: 15000
      });

      const pageTitle = page.locator('[data-testid="shield-title"], h1, .page-title').first();
  ...
  test.describe('Enhanced Selector Resilience', () => {
    test('should use comprehensive fallback selector strategies', async ({ page }) => {
      ...
      const eventsList = page
        .locator('[data-testid="shield-events"]')
    ...
    test('should handle dynamic content with stable selectors', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="shield-event"], .shield-event-item, .event-card', {
        timeout: 10000
      });
  ...
  test.describe('Loading State Improvements', () => {
    ...
    test('should show loading skeletons consistently', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="shield-container"], main, .page-container', {
        timeout: 5000
      });
