...
class TransparencyService {
  ...
  // ROUND 4 FIX: Enhanced transparency enforcement for auto-approval
  /**
   * Check if transparency is required for an organization
   * ROUND 4 FIX: GDPR compliance and organization-specific settings
   */
  async isTransparencyRequired(organizationId) {
    try {
      if (!flags.isEnabled('ENABLE_SUPABASE')) {
        // In mock mode, simulate GDPR requirement based on organization ID
        return organizationId && organizationId.includes('eu');
      }

      // Check organization settings for transparency requirements
      const { data, error } = await supabaseServiceClient
        .from('organizations')
      ...
      if (error) {
        logger.error('Error checking transparency requirements - defaulting to required', {
          organizationId,
          error: error.message,
          reason: 'database_query_error'
        });
        // Fail closed - require transparency if we can't verify
        return true;
      }

      // GDPR compliance - EU organizations always require transparency
      if (data.region === 'EU' || data.region === 'europe') {
        return true;
      }

      // Check organization-specific settings
      const settings = data.settings || {};
      if (settings.transparency_required !== undefined) {
        return settings.transparency_required;
      }
  ...
  /**
   * Apply transparency to a variant for auto-approval
   * ROUND 4 FIX: Enhanced transparency application with validation
   */
  async applyTransparency(variant, organizationId) {
    try {
      if (!variant || !variant.text) {
        logger.error('Invalid variant provided for transparency application', {
          organizationId,
          hasVariant: !!variant,
          hasText: !!(variant && variant.text),
          reason: 'invalid_input'
        });
        return null;
      }

      // Detect language from variant text
      ...
      logger.info('Transparency applied to variant', {
        organizationId,
        variantId: variant.id,
        detectedLanguage,
        originalLength: variant.text.length,
        finalLength: transparentVariant.text.length,
      ...
      logger.error('Error applying transparency to variant', {
        ...
        variantId: variant?.id,
        error: error.message,
        reason: 'transparency_application_error'
      });
      return null;
    }
  }
  ...
  /**
   * ROUND 4 FIX: Validate that transparency indicators are present
   */
  validateTransparencyCompliance(text) {
    ...
    logger.debug('Transparency compliance validation', {
      textLength: text.length,
      hasIndicator,
      checkedIndicators: indicators.length
    });
  ...
  /**
   * ROUND 4 FIX: Get transparency settings for an organization
   */
  async getTransparencySettings(organizationId) {
    try {
      if (!flags.isEnabled('ENABLE_SUPABASE')) {
        return {
          required: organizationId && organizationId.includes('eu'),
          mode: 'automatic',
          region: organizationId && organizationId.includes('eu') ? 'EU' : 'US'
        };
      }
      ...
      if (error) {
        logger.error('Error getting transparency settings', {
          organizationId,
          error: error.message
        });
        // Fail closed - return most restrictive settings
        return {
          required: true,
          mode: 'automatic',
          region: 'EU'
