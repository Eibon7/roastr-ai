...
class StyleProfileService {
  ...
  async extractStyleProfile(userId, platform, accountRef) {
    try {
      ...
      if (comments.length < this.MIN_COMMENTS_REQUIRED) {
        throw new Error(
          `Insufficient comments for style analysis. Found: ${comments.length}, Required: ${this.MIN_COMMENTS_REQUIRED}`
        );
      }
      ...
      logger.info('Style profile created', {
        ...
        profileHash: crypto
          .createHash('sha256')
          .update(JSON.stringify(styleEmbedding))
          .digest('hex')
          .substring(0, 8)
      });
  ...
  async storeStyleProfile(userId, platform, encryptedProfile) {
    ...
    const { error } = await supabaseServiceClient.from('user_style_profile').upsert(profileData, {
  ...
  async needsRefresh(userId, platform) {
    ...
    const daysSinceRefresh =
      (Date.now() - new Date(data.last_refresh).getTime()) / (1000 * 60 * 60 * 24);
