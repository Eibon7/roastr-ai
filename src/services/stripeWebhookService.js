...
class StripeWebhookService {
  ...
  async processWebhookEvent(event) {
    ...
    try {
      ...
      const { data: isProcessed } = await supabaseServiceClient.rpc('is_webhook_event_processed', {
        event_id: event.id
      });
      ...
      const { data: webhookId } = await supabaseServiceClient.rpc(
        'start_webhook_event_processing',
        {
          event_id: event.id,
      ...
      await supabaseServiceClient.rpc('complete_webhook_event_processing', {
        event_id: event.id,
  ...
  async _handleCheckoutCompleted(session) {
    try {
      ...
      const { data: transactionResult, error: transactionError } = await supabaseServiceClient.rpc(
      ...
      const success =
        transactionResult?.subscription_updated && transactionResult?.entitlements_updated;
    ...
    try {
      ...
      const { data: transactionResult, error: transactionError } = await supabaseServiceClient.rpc(
        'execute_payment_failed_transaction',
        {
          ...
          p_next_attempt_date: invoice.next_payment_attempt
  ...
  _extractSubscriptionID(event) {
    ...
    return obj.subscription || obj.id || obj.lines?.data?.[0]?.subscription || null;
  }
  ...
  async getWebhookStats(sinceDaysAgo = 1) {
    try {
      ...
      const { data, error } = await supabaseServiceClient.rpc('get_webhook_stats', {
  ...
  async cleanupOldEvents(olderThanDays = 30) {
    try {
      const { data, error } = await supabaseServiceClient.rpc('cleanup_webhook_events', {
