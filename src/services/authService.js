...
class AuthService {
  ...
  async updatePasswordWithVerification(accessToken, currentPassword, newPassword) {
    try {
      ...
      const isPasswordReused = await passwordHistoryService.isPasswordRecentlyUsed(
        user.id,
        newPassword
      );
      if (isPasswordReused) {
  ...
  async updateUserPlan(userId, newPlan, adminId = null) {
    ...
    try {
      ...
      if (!validation.allowed) {
        throw new Error(
          `Plan change not allowed: ${validation.reason}${validation.warnings?.length ? '. Warnings: ' + validation.warnings.join(', ') : ''}`
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Plan change failed during limits application and was rolled back: ${limitsError.message}`
        );
      }
    ...
    try {
      ...
      if (originalUserData?.plan) {
        try {
          ...
          logger.error(
            'Failed to restore original plan limits during rollback:',
            limitsRollbackError.message
          );
  ...
  async toggleUserActive(userId, adminUserId) {
    try {
      ...
      await this.logUserActivity(
        userId,
        newActiveStatus ? 'account_activated' : 'account_deactivated',
        {
  ...
  async logUserActivity(userId, activityType, metadata = {}) {
    try {
      ...
      // Insert activity log
      const { error: logError } = await supabaseServiceClient.from('user_activities').insert({
        user_id: userId,
  ...
  getFallbackPlanLimits(plan) {
    ...
    return {
      ...
      monthly_tokens:
        planLimits.monthlyTokensLimit === -1 ? -1 : planLimits.monthlyTokensLimit || 100000,
      integrations:
        planLimits.integrationsLimit === -1
          ? -1
          : planLimits.integrationsLimit || planLimits.maxPlatforms || 1,
      monthly_analysis:
        planLimits.monthlyAnalysisLimit === -1 ? -1 : planLimits.monthlyAnalysisLimit || 1000
    };
  }
  ...
  async requestAccountDeletion(userId) {
    try {
      ...
      if (user.deletion_scheduled_at) {
        ...
        throw new Error(
          `Account deletion is already scheduled in ${daysRemaining} days. You can cancel it from your settings.`
        );
      }
