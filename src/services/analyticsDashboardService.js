...
class AnalyticsDashboardService {
  ...
  async getBillingAnalytics({ user, rangeDays = 90 }) {
    ...
    const localCosts = await this._aggregateLocalBilling(
  ...
  async exportAnalytics({
    ...
    if (!EXPORT_ALLOWED_PLANS.has(context.planId)) {
      throw new AnalyticsPermissionError(
        'Tu plan no permite exportar analytics. Mejora a Pro o superior.'
      );
    }
  ...
  _buildSummary(snapshots, timeline) {
    ...
    if (roastsTrend > 0) {
      highlights.push(
        `Los roasts crecieron ${roastsTrend.toFixed(1)}% respecto al periodo anterior.`
      );
    }
    if (approvalRate) {
      ...
      datasets: [
        {
          ...
          backgroundColor: labels.map(
            (_, index) => PLATFORM_COLORS[index % PLATFORM_COLORS.length]
          ),
          borderWidth: 0
        }
      ]
  ...
  _buildCostOverview(snapshots, usageRecords, summary) {
    const snapshotCost = snapshots.reduce(
      (sum, snapshot) => sum + this._toNumber(snapshot.total_cost_cents),
      0
    );
    const usageCost = usageRecords.reduce(
      (sum, record) => sum + this._toNumber(record.cost_cents),
      0
    );
  ...
  async _fetchPolarBilling(startDate, endDate) {
    ...
    const refunds = paidOrders.reduce(
      (sum, order) => sum + this._toNumber(order.refundedAmount),
      0
    );
