...
class AddonService {
  ...
  async getUserAddonCredits(userId, category) {
    try {
      const { data: credits, error } = await supabaseServiceClient.rpc('get_user_addon_credits', {
        p_user_id: userId,
  ...
  async consumeAddonCredits(userId, category, amount = 1) {
    try {
      ...
      const { data: success, error } = await supabaseServiceClient.rpc('consume_addon_credits', {
        p_user_id: userId,
        p_addon_category: category,
        p_amount: amount
      });
  ...
  async userHasFeatureAddon(userId, featureKey) {
    try {
      const { data: hasFeature, error } = await supabaseServiceClient.rpc(
  ...
  async canPerformAction(userId, action, planLimits, currentUsage) {
    try {
      ...
      const planLimit =
        planLimits[`monthly_${action}s_limit`] || planLimits.monthly_responses_limit || 0;
