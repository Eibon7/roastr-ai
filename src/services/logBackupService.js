...
class LogBackupService {
  ...
  initializeS3() {
    try {
      ...
      if (!this.bucketName) {
        advancedLogger.warn(
          'S3 backup not configured - LOG_BACKUP_S3_BUCKET environment variable not set'
        );
        return;
      }
  ...
  isRetryableError(error) {
    ...
    return (
      retryableErrors.some((err) => error.code === err || error.name === err) ||
    ...
    for (let i = 0; i < days; i++) {
      ...
      try {
        ...
        advancedLogger.error(
          `Failed to backup logs for ${currentDate.toISOString().split('T')[0]}`,
          {
            error: error.message
          }
        );
  ...
  async listBackups(options = {}) {
    ...
    const { prefix = this.backupPrefix, maxKeys = 1000, date = null } = options;
  ...
  async cleanOldBackups(retentionDays = 90, options = {}) {
    ...
    try {
      ...
      const toDelete = backups.backups.filter(
        (backup) => new Date(backup.lastModified) < cutoffDate
      );
        ...
        if (dryRun) {
          ...
          try {
            ...
            advancedLogger.info('Deleted old backup', {
              key: backup.key,
              age:
                Math.floor((Date.now() - new Date(backup.lastModified)) / (1000 * 60 * 60 * 24)) +
                ' days'
            });
          } catch (error) {
            results.errors.push({ key: backup.key, error: error.message });
            advancedLogger.error('Failed to delete backup', {
              key: backup.key,
              error: error.message
            });
          }
  ...
  generateBackupSummary(results) {
    ...
    summary.successRate =
      (((summary.totalDays - summary.errorDates.length) / summary.totalDays) * 100).toFixed(1) +
      '%';
