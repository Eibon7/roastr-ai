...
const { SECURITY_CONFIG, VALIDATION_HELPERS, CACHE_CONFIG } = require('../config/tierConfig');
...
class PlanLimitsService {
  ...
  async getPlanLimits(planId) {
    try {
      ...
      if (!defaultLimits) {
        logger.warn('Plan not found in planService.js, using starter_trial as fallback', {
          planId
        });
      ...
      if (!error && data) {
        const dbLimits = {
          ...
          monthlyResponsesLimit:
            data.monthly_responses_limit ?? defaultLimits.monthlyResponsesLimit,
      ...
      if (!error && data && data.length > 0) {
        ...
        data.forEach((planLimit) => {
          ...
          mergedLimits[planId] = {
            ...
            monthlyResponsesLimit:
              planLimit.monthly_responses_limit ?? defaults.monthlyResponsesLimit,
  ...
  async updatePlanLimits(planId, updates, updatedBy) {
    try {
      ...
      if (updates.monthlyResponsesLimit !== undefined)
        dbUpdates.monthly_responses_limit = updates.monthlyResponsesLimit;
      if (updates.monthlyAnalysisLimit !== undefined)
        dbUpdates.monthly_analysis_limit = updates.monthlyAnalysisLimit;
      if (updates.maxPlatforms !== undefined) dbUpdates.max_platforms = updates.maxPlatforms;
      if (updates.integrationsLimit !== undefined)
        dbUpdates.integrations_limit = updates.integrationsLimit;
      ...
      if (updates.prioritySupport !== undefined)
        dbUpdates.priority_support = updates.prioritySupport;
      if (updates.apiAccess !== undefined) dbUpdates.api_access = updates.apiAccess;
      if (updates.analyticsEnabled !== undefined)
        dbUpdates.analytics_enabled = updates.analyticsEnabled;
      if (updates.customTones !== undefined) dbUpdates.custom_tones = updates.customTones;
      if (updates.dedicatedSupport !== undefined)
        dbUpdates.dedicated_support = updates.dedicatedSupport;
      if (updates.monthlyTokensLimit !== undefined)
        dbUpdates.monthly_tokens_limit = updates.monthlyTokensLimit;
      if (updates.dailyApiCallsLimit !== undefined)
        dbUpdates.daily_api_calls_limit = updates.dailyApiCallsLimit;
  ...
  async checkLimit(planId, limitType, currentUsage) {
    try {
      ...
      const isProductionOrSecure =
        process.env.NODE_ENV === 'production' || process.env.PLAN_LIMITS_FAIL_CLOSED === 'true';
      ...
      const allowFailOpen =
        process.env.NODE_ENV !== 'production' && process.env.PLAN_LIMITS_FAIL_OPEN === 'true';
