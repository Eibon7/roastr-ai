...
class RoastEngine {
  ...
  async generateRoast(input, options = {}) {
    ...
    try {
      logger.info('🔥 Starting roast generation with engine', {
      ...
      // Get roast versions flag (2 → 1 version control)
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          logger.info(`🔄 Roast generation attempt ${attempt}/${maxRetries}`);
      ...
      if (!result) {
        throw new Error(
          `Roast generation failed after ${maxRetries} attempts: ${lastError?.message || 'Unknown error'}`
        );
      }
      ...
      logger.error('❌ Roast generation failed completely', {
        userId: options.userId,
        error: error.message,
        stack: error.stack,
        processingTimeMs: processingTime
  ...
  validateInput(input, options) {
    ...
    if (input.comment.length > VALIDATION_CONSTANTS.MAX_COMMENT_LENGTH) {
      throw new Error(
        `Comment exceeds maximum length of ${VALIDATION_CONSTANTS.MAX_COMMENT_LENGTH} characters`
      );
    }
  ...
  async getUserConfiguration(userId) {
    try {
      const { data, error } = await supabaseServiceClient.rpc('get_user_roast_config', {
        user_uuid: userId
      });
  ...
  async handleAutoApprove(result, options) {
    ...
    try {
      logger.info('🔍 Processing auto-approve with transparency validation');
      ...
      if (!transparencyValidation.isValid) {
        ...
        logger.error('❌ Transparency validation failed - blocking publication', {
      ...
      logger.info('✅ Auto-approve completed with transparency validation', {
        userId: options.userId,
  ...
  async persistMetadata(result, input, options) {
    try {
      ...
      if (error) {
        ...
        logger.info('✅ Roast metadata persisted successfully', {
          id: metadata.id,
          userId: options.userId
        });
      }
