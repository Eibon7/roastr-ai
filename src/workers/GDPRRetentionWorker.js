...
class GDPRRetentionWorker extends BaseWorker {
  constructor(options = {}) {
    ...
    if (!this.supabase) {
      throw new Error(
        'GDPRRetentionWorker requires a Supabase client with service-role permissions'
      );
    }
  ...
  async cleanupOldProfiles(batchId) {
    ...
    try {
      ...
      const { data, error } = await this.supabase.rpc('cleanup_old_offender_profiles');
  ...
  async logRetentionOperation(logData) {
    try {
      const { error } = await this.supabase.from('shield_retention_log').insert({
        ...logData,
