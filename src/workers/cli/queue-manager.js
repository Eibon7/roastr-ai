      ...
      for (const jobType of jobTypes) {
        ...
        if (length > 0) {
          ...
          jobs.forEach((jobData, index) => {
            ...
            console.log(`  [${index + 1}] ${job.id} - Failed: ${job.failed_at} - Error: ${job.final_error?.substring(0, 100)}...`);
          });
  ...
  try {
    ...
    switch (command) {
      ...
      case 'clear':
        ...
        if (!jobType) {
          console.error('❌ Please specify job type (fetch_comments, analyze_toxicity, generate_reply, shield_action)');
          process.exit(1);
        }
