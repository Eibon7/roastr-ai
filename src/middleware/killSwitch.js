  ...
  constructor(options = {}) {
    ...
    this.localCacheConfig = {
      ...
      encryptionKey:
        ...
        process.env.KILL_SWITCH_CACHE_KEY ||
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42P01' || /does not exist/i.test(msg)) {
          logger.warn(
            '⚠️  Kill switch table not found - feature disabled until migration applied',
            {
              error: error.message,
          ...
          this.cache.set('KILL_SWITCH_AUTOPOST', {
            is_enabled: false,
            flag_value: false,
            cached_at: Date.now()
          });
          this.cache.set('ENABLE_AUTOPOST', {
            is_enabled: false,
            flag_value: false,
            cached_at: Date.now()
          });
  ...
  handleMissingFlag(flagKey, error = null) {
    ...
    switch (this.config.missingFlagBehavior) {
      ...
      case 'throw':
        ...
        throw new Error(
          `Feature flag '${flagKey}' not found and missingFlagBehavior is set to 'throw'`
        );
