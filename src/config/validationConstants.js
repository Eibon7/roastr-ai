...
function getValidStylesForLanguage(language = 'es') {
  const normalizedLanguage = normalizeLanguage(language);
  return (
    VALIDATION_CONSTANTS.VALID_STYLES[normalizedLanguage] || VALIDATION_CONSTANTS.VALID_STYLES.es
  );
}
...
function normalizeIntensity(intensity) {
  ...
  if (
    normalized < VALIDATION_CONSTANTS.MIN_INTENSITY ||
    normalized > VALIDATION_CONSTANTS.MAX_INTENSITY
  ) {
    return null;
  }
