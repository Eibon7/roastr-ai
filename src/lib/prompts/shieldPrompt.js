...
`;
  ...
  buildBlockB(options = {}) {
    ...
    if (redLines && typeof redLines === 'object') {
      ...
      if (redLines.categories && Array.isArray(redLines.categories) && redLines.categories.length > 0) {
    ...
    if (shieldSettings && typeof shieldSettings === 'object') {
      if (shieldSettings.aggressiveness && typeof shieldSettings.aggressiveness === 'number') {
        ...
        const strictnessNote = shieldSettings.aggressiveness >= 0.98 
          ...
          : '';
        parts.push(`Classification strictness: ${shieldSettings.aggressiveness}${strictnessNote}\n`);
      }
    }
