  ...
  try {
    ...
    const formattedResponses = await Promise.all(
      pendingResponses.map(async (response) => {
        ...
        const { data: attemptCount } = await supabaseServiceClient.rpc('count_roast_attempts', {
          comment_uuid: response.comments.id
        });
...
router.post('/:id/approve', async (req, res) => {
  try {
    ...
    if (queueError) {
      ...
      if (rollbackError) {
        logger.error(
          'Critical: Failed to rollback response approval after queue error:',
    ...
    const { error: historyError } = await supabaseServiceClient.from('roast_attempts').insert({
      organization_id: orgData.id,
...
router.post('/:id/reject', async (req, res) => {
  try {
    ...
    logger.info(
      `Response rejected: ${id} by user ${user.id}${reason ? ` (reason: ${reason})` : ''}`
    );
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        posted: posted,
        approval_rate:
          totalResponses > 0 ? (((approved + posted) / totalResponses) * 100).toFixed(1) : 0
      }
    });
  } catch (error) {
...
router.post('/:id/regenerate', async (req, res) => {
  try {
    ...
    const { data: variantCount, error: countError } = await supabaseServiceClient.rpc(
      'count_roast_attempts',
      { comment_uuid: originalResponse.comment_id }
    );
    ...
    if (countError) {
      ...
      logger.warn(
        `Variant limit reached for comment ${originalResponse.comment_id}: ${variantCount} attempts`
      );
    ...
    const { data: nextAttemptNum, error: attemptError } = await supabaseServiceClient.rpc(
    ...
    if (discardError) {
      ...
      if (rollbackError) {
        logger.error(
          'CRITICAL: Failed to rollback new response after discard error:',
          rollbackError.message
        );
      }
    ...
    logger.info(
      `Response regenerated: ${id} -> ${newResponse.id} by user ${user.id} (attempt ${nextAttemptNum})`
    );
