...
router.get('/pending', async (req, res) => {
    try {
        ...
        // Get attempt counts for each comment
        const commentIds = pendingResponses.map(r => r.comments.id);
        const { data: attemptCounts } = await supabaseServiceClient
            .rpc('count_roast_attempts', { comment_uuid: commentIds[0] }); // For now, get one at a time
