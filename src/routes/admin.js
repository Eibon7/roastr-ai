...
} = require('../middleware/responseCache'); // M1 fix
...
router.get('/users', cacheResponse({ ttl: 30 * 1000 }), async (req, res) => {
  try {
    ...
    let query = supabaseServiceClient.from('users').select(
      `
    ...
    const usersWithHandles = (users || []).map((user) => {
      try {
        ...
        const handles = userIntegrations
          .map((integration) => `@${integration.handle || 'unknown'} (${integration.platform})`)
          .join(', ');
...
router.get('/usage', async (req, res) => {
  try {
    ...
    let query = supabaseServiceClient.from('usage_tracking').select(`
    ...
    const { data: summaryData, error: summaryError } = await supabaseServiceClient.rpc(
      'get_usage_summary',
      {
        target_year: period === 'last_month' && currentMonth === 1 ? currentYear - 1 : currentYear,
        target_month:
          period === 'last_month' ? (currentMonth === 1 ? 12 : currentMonth - 1) : currentMonth
      }
    );
...
router.get('/usage/export', async (req, res) => {
  try {
    const { period = 'current_month', format = 'csv', organization_id = null } = req.query;
    ...
    if (format === 'csv') {
      // Generate CSV
      let csv =
        'Organization,Plan,Resource Type,Platform,Quantity,Cost (cents),Tokens Used,Date,Created At\n';
...
router.get('/logs/download', async (req, res) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="roastr-logs-${new Date().toISOString().split('T')[0]}.txt"`
    );
...
router.patch('/users/:userId/config', async (req, res) => {
  try {
    ...
    if (persona) {
      ...
      if (persona.doesntTolerate !== undefined)
        updates.persona_doesnt_tolerate = persona.doesntTolerate;
    ...
    // Security audit logging
    await auditLogger.logAdminUserModification(req.user.id, userId, updates, req.user.email);
...
router.get('/plans', async (req, res) => {
  try {
    ...
    const plansData = await Promise.all(
      Object.entries(allPlans).map(async ([planId, plan]) => {
        const limits = await planLimitsService.getPlanLimits(planId);
        return {
          id: planId,
          name: plan.name,
...
router.put('/plans/:planId', async (req, res) => {
  try {
    ...
    if (updates.maxPlatforms !== undefined)
      planUpdates.maxPlatforms = parseInt(updates.maxPlatforms);
    if (updates.monthlyAnalysisLimit !== undefined)
      planUpdates.monthlyAnalysisLimit = parseInt(updates.monthlyAnalysisLimit);
    if (updates.monthlyTokensLimit !== undefined)
      planUpdates.monthlyTokensLimit = parseInt(updates.monthlyTokensLimit);
    if (updates.platformIntegrations !== undefined)
      planUpdates.integrationsLimit = parseInt(updates.platformIntegrations);
