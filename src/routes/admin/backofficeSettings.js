...
router.post('/healthcheck', async (req, res) => {
  try {
    ...
    const { error: storeError } = await supabaseServiceClient.from('healthcheck_results').insert({
    ...
    res.json({
      ...
      data: {
        results: healthResults,
        overall_status: Object.values(healthResults).every((r) => r.status === 'OK')
          ? 'OK'
          : 'FAIL',
        checked_at: new Date().toISOString(),
        platforms_checked: platformsToCheck
...
router.get('/audit/export', async (req, res) => {
  try {
    const { format = 'csv', days = 30, action_type, resource_type, admin_user_id } = req.query;
    ...
    if (format === 'csv') {
      // Generate CSV
      let csv =
        'Timestamp,Admin Email,Action Type,Resource Type,Resource ID,Description,IP Address,User Agent,Old Value,New Value\n';
...
async function logAdminAction(
  ...
  try {
    const { error } = await supabaseServiceClient.from('admin_audit_logs').insert({
