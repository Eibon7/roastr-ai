...
router.get('/user/addons', requireShopEnabled, authenticateToken, async (req, res) => {
  try {
    ...
    const { data: roastCredits, error: roastCreditsError } = await supabaseServiceClient.rpc(
      'get_user_addon_credits',
      {
        p_user_id: userId,
    ...
    const { data: analysisCredits, error: analysisCreditsError } = await supabaseServiceClient.rpc(
      'get_user_addon_credits',
      {
        p_user_id: userId,
    ...
    const { data: rqcEnabled, error: rqcError } = await supabaseServiceClient.rpc(
      'user_has_feature_addon',
      {
        p_user_id: userId,
...
router.post('/checkout', requireShopEnabled, authenticateToken, async (req, res) => {
  try {
    ...
    idempotencyKey = idempotencyKey ? String(idempotencyKey).slice(0, 255) : crypto.randomUUID();
    logger.debug('Using idempotency key for checkout session', {
      userId,
