...
async function getPlanLimitsHealth() {
  try {
    ...
    if (health.lastRefresh && Date.now() - health.lastRefresh > 10 * 60 * 1000) {
      // 10 minutes
...
router.get('/health', async (req, res) => {
  try {
    ...
    const httpStatus =
      tierHealth.status === 'unhealthy' ? 503 : tierHealth.status === 'degraded' ? 200 : 200;
...
router.put('/alerts/config', requireAdmin, async (req, res) => {
  try {
    ...
    const validKeys = [
      'errorRatePercentage',
      'criticalErrorRatePercentage',
      'maxErrorsPerHour',
      'maxValidationsPerMinute'
    ];
...
router.post('/alerts/test', requireAdmin, async (req, res) => {
  try {
    const {
      severity = 'info',
      title = 'Test Alert',
      message = 'This is a test alert from monitoring system'
    } = req.body;
