...
router.post('/accounts/:id/roasts/:roastId/regenerate', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await integrationsService.regenerateRoast(userId, id, roastId, {
      tone,
      intensity
    });
...
router.patch('/accounts/:id/settings', authenticateToken, async (req, res) => {
  try {
    ...
    if (validSettings.shieldLevel !== undefined) {
      if (
        typeof validSettings.shieldLevel !== 'number' ||
        validSettings.shieldLevel < 1 ||
        validSettings.shieldLevel > 100
      ) {
        return res.status(400).json({
          success: false,
    ...
    if (validSettings.defaultTone !== undefined) {
      // Trim whitespace and normalize tone to canonical form (handles case variations)
      const trimmedTone =
        typeof validSettings.defaultTone === 'string'
          ? validSettings.defaultTone.trim()
          : validSettings.defaultTone;
      const normalizedTone = normalizeTone(trimmedTone);

      if (!normalizedTone) {
        return res.status(400).json({
          success: false,
          error: `Tono inválido. Debe ser uno de: ${VALID_TONES.join(', ')}`
        });
      }
  ...
  try {
    ...
    if (flags.isEnabled('ENABLE_SUPABASE')) {
      ...
      if (!req.accessToken || typeof req.accessToken !== 'string') {
        logger.error('Missing or invalid access token for user client creation', {
      ...
      logger.info('Mock mode: User preferences updated', {
        userId: userId?.substr(0, 8) + '...' || 'unknown',
        humor_tone,
        humor_style
      });
      ...
      if (!orgError && userData.organizations[0]) {
        ...
        const integrationPromises = preferred_platforms.map(async (platform) => {
          ...
          if (!existing) {
            ...
            if (createError) {
              logger.warn(
                `Failed to create integration config for ${platform}:`,
                createError.message
              );
              return null;
            }
      ...
      if (flags.isEnabled('ENABLE_SUPABASE')) {
        try {
          ...
          if (passwordError) {
            ...
            await auditService.logAccountDeletionAttempt(
              userId,
              {
                success: false,
    ...
    try {
      ...
      await auditService.logAccountDeletionCancellation(
        ...
        deletionRequest.id,
        {
          ...
          cancelled_within_hours: Math.round(
            (scheduledDeletion.getTime() - now.getTime()) / (1000 * 60 * 60)
          )
        },
        req
      );
    ...
    try {
      ...
      if (!trustedOrigin) {
        ...
        try {
          ...
          throw new Error(
            `Invalid APP_PUBLIC_URL or PUBLIC_BASE_URL: failed to parse "${trustedOrigin}": ${error.message}`
          );
        }
        ...
        if (!['http:', 'https:'].includes(parsedOrigin.protocol)) {
          throw new Error(
            `Invalid APP_PUBLIC_URL or PUBLIC_BASE_URL: protocol must be http or https, got "${parsedOrigin.protocol}" in "${trustedOrigin}"`
          );
        }
        ...
        if (parsedOrigin.username || parsedOrigin.password) {
          throw new Error(
            `Invalid APP_PUBLIC_URL or PUBLIC_BASE_URL: must not contain credentials in "${trustedOrigin}"`
          );
        }
        ...
        if (parsedOrigin.search || parsedOrigin.hash) {
          throw new Error(
            `Invalid APP_PUBLIC_URL or PUBLIC_BASE_URL: must not contain query or fragment in "${trustedOrigin}"`
          );
        }
        ...
        if (parsedOrigin.pathname !== '' && parsedOrigin.pathname !== '/') {
          throw new Error(
            `Invalid APP_PUBLIC_URL or PUBLIC_BASE_URL: must not contain path, got "${parsedOrigin.pathname}" in "${trustedOrigin}"`
          );
        }
        ...
        if (!exportResult.downloadUrl || typeof exportResult.downloadUrl !== 'string') {
          throw new Error(
            `Invalid download URL: must be a string, got: ${typeof exportResult.downloadUrl}`
          );
        }
        ...
        try {
          ...
          throw new Error(
            `Invalid download URL: failed to decode path "${exportResult.downloadUrl}": ${error.message}`
          );
        }
        ...
        if (!normalizedPath.startsWith('/')) {
          throw new Error(
            `Invalid download URL: must start with '/', got: "${exportResult.downloadUrl}"`
          );
        }

        if (normalizedPath.includes('..')) {
          throw new Error(
            `Invalid download URL: path traversal detected in "${exportResult.downloadUrl}"`
          );
        }

        if (normalizedPath.includes('\0')) {
          throw new Error(
            `Invalid download URL: null byte detected in "${exportResult.downloadUrl}"`
          );
        }
        ...
        for (const pattern of suspiciousPatterns) {
          if (lowerPath.includes(pattern)) {
            throw new Error(
              `Invalid download URL: suspicious encoding detected in "${exportResult.downloadUrl}"`
            );
          }
        }
        ...
        if (normalizedPath.substring(1).includes('//')) {
          throw new Error(
            `Invalid download URL: double slashes detected in "${exportResult.downloadUrl}"`
          );
        }
        ...
        try {
          ...
          throw new Error(
            `Failed to construct valid download URL from origin "${trustedOrigin}" and path "${exportResult.downloadUrl}": ${error.message}`
          );
        }
      }
...
router.post('/roastr-persona/validate', authenticateToken, async (req, res) => {
  try {
    ...
    const { data: existingData } = await supabaseServiceClient
      .from('users')
      .select(
        'lo_que_me_define_encrypted, lo_que_no_tolero_encrypted, lo_que_me_da_igual_encrypted'
      )
      .eq('id', userId)
      .single();
    ...
    if (existingData) {
      ...
      if (field !== 'loQueMeDaIgual' && existingData.lo_que_me_da_igual_encrypted) {
        try {
          ...
          logger.warn('Failed to decrypt lo_que_me_da_igual for length check', {
            error: e.message
          });
        }
      }
  ...
  try {
    ...
    if (flags.isEnabled('ENABLE_SUPABASE')) {
      ...
      if (existingError && existingError.code !== 'PGRST116') {
        ...
        return res
          .status(500)
          .json({ success: false, error: 'Failed to retrieve existing persona data' });
      }
      ...
      if (loQueMeDefine !== undefined) {
        ...
        if (
          ...
          const encryptionSanitized =
            encryptionService.sanitizeForEncryption(sanitizedLoQueMeDefine);
          try {
      ...
      if (loQueNoTolero !== undefined) {
        ...
        if (
          ...
          // Encrypt the sanitized and validated content
          const encryptionSanitized =
            encryptionService.sanitizeForEncryption(sanitizedLoQueNoTolero);
          try {
      ...
      if (loQueMeDaIgual !== undefined) {
        ...
        if (
          ...
          // Encrypt the sanitized and validated content
          const encryptionSanitized =
            encryptionService.sanitizeForEncryption(sanitizedLoQueMeDaIgual);
          try {
            updateData.lo_que_me_da_igual_encrypted =
              encryptionService.encrypt(encryptionSanitized);
      ...
      const { data: updateResult, error: updateError } = await userClient.rpc(
        'update_roastr_persona_transactional',
        {
          p_user_id: userId,
      ...
      await generateEmbeddingsForPersona(
        userId,
        {
      ...
      if (updatedUser.lo_que_me_da_igual_encrypted) {
        try {
          responseLoQueMeDaIgual = encryptionService.decrypt(
            updatedUser.lo_que_me_da_igual_encrypted
          );
        } catch (decryptError) {
    ...
    try {
      ...
      if (flags.isEnabled('ENABLE_SUPABASE')) {
        ...
        const { data: deleteResult, error: deleteError } = await userClient.rpc(
          'delete_roastr_persona_transactional',
          {
            p_user_id: userId,
            p_field_type: field
          }
        ...
        const updatedUser = {
          ...
          lo_que_no_tolero_updated_at:
            field === 'intolerance' || field === 'all' ? timestamp : null,
        ...
        res.json({
          ...
          data: {
            ...
            deletedAt: timestamp,
            identityDeletedAt:
              field === 'identity' || field === 'all'
                ? updatedUser.lo_que_me_define_updated_at
                : null,
            intoleranceDeletedAt:
              field === 'intolerance' || field === 'all'
                ? updatedUser.lo_que_no_tolero_updated_at
                : null,
            toleranceDeletedAt:
              field === 'tolerance' || field === 'all'
                ? updatedUser.lo_que_me_da_igual_updated_at
                : null
          }
        });
      } else {
...
async function generateEmbeddingsForPersona(userId, personaData, userClient) {
  try {
    ...
    if (Object.keys(updateData).length > 3) {
      // More than just metadata fields
      ...
      if (embeddingUpdateError) {
        ...
        logger.info('Successfully generated and stored persona embeddings', {
          userId: SafeUtils.safeUserIdPrefix(userId),
          embeddingsGenerated: Object.keys(updateData).filter(
            (key) => key.includes('embedding') && updateData[key] !== null
          ).length,
          embeddingsCleared: Object.keys(updateData).filter(
            (key) => key.includes('embedding') && updateData[key] === null
          ).length,
    ...
    if (flags.isEnabled('ENABLE_SUPABASE')) {
      ...
      res.json({
        ...
        data: {
          ...
          bio_text:
            !data.transparency_mode || data.transparency_mode === 'bio'
