...
async function recordRoastUsage(userId, metadata = {}) {
  try {
    await supabaseServiceClient.from('roast_usage').insert({
      user_id: userId,
      count: 1,
      metadata,
...
async function recordAnalysisUsage(userId, metadata = {}) {
  try {
    await supabaseServiceClient.from('analysis_usage').insert({
      user_id: userId,
      count: 1,
      metadata,
      created_at: new Date().toISOString()
...
async function consumeRoastCredits(userId, plan, metadata = {}) {
  try {
    ...
    const { data: result, error } = await supabaseServiceClient.rpc('consume_roast_credits', {
      p_user_id: userId,
      p_plan: plan,
