...
class TwitchService extends BaseIntegration {
  ...
  async handleChatMessage(channel, user, text, messageInfo) {
    try {
      ...
      if (
        this.subscriberOnly &&
        !messageInfo.userInfo.isSubscriber &&
        !messageInfo.userInfo.isMod
      ) {
        return;
      }
      ...
      const isMentioned = text
        .toLowerCase()
        .includes(this.botUsername?.toLowerCase() || 'roastbot');
  ...
  async handleRoastCommand(channel, user, args, messageInfo) {
    try {
      const target = args.length > 0 ? args.join(' ') : 'your request for a roast';
  ...
  async handleStatsCommand(channel, user, args, messageInfo) {
    ...
    await this.chatClient.say(channel, `@${user} Roast stats coming soon! 📊`);
  }
