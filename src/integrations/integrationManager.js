...
class IntegrationManager {
  ...
  async initializeIntegrations() {
    try {
      ...
      logger.info(`✅ Integration Manager initialized: ${successCount} successful, ${failureCount} failed`);
        ...
        try {
          ...
          if (typeof integration.runOnce === 'function') {
            ...
            if (typeof integration.testConnection === 'function') {
              ...
              logger.info(`✅ ${platform} fetch test completed (${comments?.length || 0} comments would be processed)`);
              successCount++;
            } else {
              logger.warn(`⚠️ ${platform} doesn't implement runOnce() or testConnection() - test skipped`);
              failureCount++;
            }
          }
        } catch (error) {
