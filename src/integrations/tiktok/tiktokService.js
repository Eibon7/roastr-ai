...
class TikTokService extends MultiTenantIntegration {
  constructor(options = {}) {
    ...
    this.triggerWords = (
      process.env.TIKTOK_TRIGGER_WORDS || 'roast,savage,fire,slay,queen,king'
    ).split(',');
  ...
  async refreshAccessToken() {
    try {
      const response = await this.makeRequest(
        ...
        false
      ); // Don't use current access token for this request
  ...
  async makeRequest(endpoint, method = 'GET', params = {}, useAuth = true) {
    ...
    try {
      ...
      if (error.response) {
        throw new Error(
          `TikTok API error: ${error.response.data.error?.message || error.response.statusText}`
        );
      } else {
