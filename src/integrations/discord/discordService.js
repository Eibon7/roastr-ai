...
class DiscordService extends MultiTenantIntegration {
  constructor(options = {}) {
    ...
    this.monitoredChannels = (process.env.DISCORD_MONITORED_CHANNELS || '')
      .split(',')
      .filter(Boolean);

    // Discord-specific settings
    this.triggerWords = (
      process.env.DISCORD_TRIGGER_WORDS || 'roast,burn,insult,comeback,bot'
    ).split(',');
  ...
  async handleMessage(message) {
    ...
    if (this.monitoredChannels.length > 0 && !this.monitoredChannels.includes(message.channel.id)) {
      return;
    }
  ...
  async handleRoastCommand(interaction) {
    const targetMessage = interaction.options.getString('message') || 'Your request for a roast';
