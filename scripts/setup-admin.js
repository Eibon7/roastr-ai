...
async function createAdminUser() {
  ...
  try {
    ...
    const password = await question(
      '🔒 Password temporal (opcional, se generará uno si está vacío): '
    );
    ...
    if (authError) {
      ...
      if (
        ...
        const { data: existingUsers, error: searchError } =
          await supabaseServiceClient.auth.admin.listUsers();
...
async function listAdmins() {
  ...
  try {
    ...
    admins.forEach((admin, index) => {
      ...
      console.log(
        `   ${admin.active ? '✅' : '❌'} Estado: ${admin.active ? 'Activo' : 'Inactivo'}`
      );
      console.log(`   📅 Creado: ${new Date(admin.created_at).toLocaleDateString('es-ES')}`);
      console.log(
        `   🕐 Última actividad: ${admin.last_activity_at ? new Date(admin.last_activity_at).toLocaleDateString('es-ES') : 'Nunca'}`
      );
      console.log('');
    });
  } catch (error) {
