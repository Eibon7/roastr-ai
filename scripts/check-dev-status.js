...
const frontendRunning = checkPort(
  expectedFrontendPort,
  `Frontend (PRIMARY - Port ${expectedFrontendPort})`
);
...
if (port3002) {
  ...
  if (process.platform === 'win32') {
    console.log(
      '   Consider stopping processes on port 3002: `netstat -ano | findstr :3002` then `taskkill /PID <PID> /F`'
    );
  } else {
...
console.log('\n🌐 Development URLs:');
console.log(
  `   Frontend: ${devConfig.FRONTEND_URL || `http://localhost:${expectedFrontendPort}`} ${frontendRunning ? '(RUNNING)' : '(STOPPED)'}`
);
console.log(
  `   Backend:  ${devConfig.BACKEND_URL || `http://localhost:${expectedBackendPort}`} ${backendRunning ? '(RUNNING)' : '(STOPPED)'}`
);
