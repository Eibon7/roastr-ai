...
try {
  ...
  if (process.argv.includes('--json')) {
    ...
    console.log(JSON.stringify({
      ...
      average: avg,
      status: avg >= 70 ? 'excellent' : avg >= 50 ? 'good' : avg >= 30 ? 'needs_improvement' : 'critical'
    }, null, 2));
  } else if (process.argv.includes('--short')) {
    // One-line summary
    console.log(`Coverage: ${avg.toFixed(2)}% (Lines: ${total.lines.pct.toFixed(2)}%, Functions: ${total.functions.pct.toFixed(2)}%, Branches: ${total.branches.pct.toFixed(2)}%)`);
  } else {
