...
export const useSocialAccounts = () => {
  ...
  const roastsByAccount = useCallback(
    (accountId) => {
  ...
  const interceptedByAccount = useCallback(
    (accountId) => {
  ...
  const getConnectionLimits = useCallback(() => {
    const planTier = normalizePlanId(
      (userData.isAdminMode
        ? userData.adminModeUser?.plan || ''
        : userData?.plan || ''
      ).toLowerCase()
    );
    const maxConnectionsPerPlatform =
      planTier === 'starter_trial' || planTier === 'starter' ? 1 : 2;
  ...
  const onApproveRoast = useCallback(
    async (accountId, roastId) => {
      // Optimistic update
      const previousState = roastsData[accountId];
      setRoastsData((prev) => ({
        ...prev,
        [accountId]:
          prev[accountId]?.map((roast) =>
  ...
  const onRejectRoast = useCallback(
    async (accountId, roastId) => {
      // Optimistic update
      const previousState = roastsData[accountId];
      setRoastsData((prev) => ({
  ...
  const onToggleAccount = useCallback(
    async (accountId, nextStatus) => {
      // Optimistic update
      const previousAccounts = accounts;
      setAccounts((prev) =>
        prev.map((account) =>
          account.id === accountId ? { ...account, status: nextStatus } : account
        )
      );
  ...
  const onChangeShieldLevel = useCallback(
    async (accountId, level) => {
      // Optimistic update
      const previousAccounts = accounts;
      setAccounts((prev) =>
        prev.map((account) =>
  ...
  const onToggleShield = useCallback(
    async (accountId, nextValue) => {
      // Optimistic update
      const previousAccounts = accounts;
      setAccounts((prev) =>
