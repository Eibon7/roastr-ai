...
const AdminUsersPage = () => {
      ...
      try {
        ...
        if (
          ...
          (error.message.includes('Sesión expirada') ||
            error.message.includes('permisos de administrador'))
        ) {
          setTimeout(() => {
            navigate('/login');
          }, 3000);
  ...
  const handleViewUserDashboard = useCallback(
    (user) => {
  ...
  const handleSuspensionConfirm = useCallback(
    async (reason) => {
      ...
      <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
        ...
        <td className="px-6 py-4">
          <div className="text-sm text-gray-900 dark:text-white">
            {user.handles &&
            user.handles !== 'No connections' &&
            user.handles !== 'Error loading' ? (
              <div className="space-y-1">
                {user.handles.split(', ').map((handle, idx) => (
                  <div
                    key={idx}
                    className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300 mr-1 mb-1"
                  >
                    {handle}
                  </div>
                ))}
  ...
  return (
    <>
              ...
              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    Mostrando{' '}
                    <span className="font-medium">{(currentPage - 1) * usersPerPage + 1}</span> a{' '}
      ...
      <SuspensionModal
        ...
        isLoading={
          actionLoading[`suspend_${suspensionModal.user?.id}`] ||
          actionLoading[`unsuspend_${suspensionModal.user?.id}`]
        }
      />
            ...
            <div className="mt-3 text-center">
              ...
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  ¿Estás seguro de que quieres cambiar el plan de{' '}
                  <strong>{planChangeModal.user?.email}</strong> de{' '}
