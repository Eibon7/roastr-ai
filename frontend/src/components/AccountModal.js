...
const AccountModal = ({
    ...
    const fetchAccountData = async () => {
      try {
        ...
        const roastsResponse = await fetch(
          `/api/user/accounts/${account.platform || account.id}/roasts?limit=10`,
          {
            headers: {
              Authorization: `Bearer ${token}`
            }
  ...
  const handleEditRoast = useCallback(
    (roastId) => {
      handleAsyncAction(
        `edit-${roastId}`,
        () => {
      ...
      handleAsyncAction(
        ...
        async () => {
          ...
          const response = await fetch(
            `/api/user/accounts/${accountId}/roasts/${roastId}/regenerate`,
            {
              method: 'POST',
              headers: {
  ...
  const handlePublishRoast = useCallback(
    (roastId) => {
      handleAsyncAction(
        `publish-${roastId}`,
        async () => {
  ...
  const handleDiscardRoast = useCallback(
    (roastId) => {
      handleAsyncAction(
        `discard-${roastId}`,
        async () => {
                ...
                <div>
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      />
                      <span
                        className={`text-sm font-medium ${isActive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                      >
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      {formatRoastCount(
                        accountDetails?.monthlyRoasts || account.monthlyRoasts || 0
                      )}{' '}
                      roasts/mes
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center space-x-2">
                        <div
                          className={`w-3 h-3 rounded-full ${accountDetails?.settings?.shieldEnabled ? 'bg-green-500' : 'bg-gray-400'}`}
                        />
                        <span
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      Shield protege automáticamente tu cuenta interceptando comentarios tóxicos y
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
                          accountDetails?.settings?.autoApprove
                            ? 'bg-primary-600'
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button
                        onClick={() =>
                          onToggleShield(
                            account.platform || account.id,
                            !(accountDetails?.settings?.shieldEnabled || false)
                          )
                        }
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
                          accountDetails?.settings?.shieldEnabled
                  ...
                  <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
                    <div className="mb-4">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          onChangeTone(account.platform || account.id, e.target.value)
                        }
                        className="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                      >
                    ...
                    <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-3">
                      <p className="text-sm text-purple-900 dark:text-purple-100 font-medium mb-1">
                        Ejemplo de roast{' '}
                        {(accountDetails?.settings?.defaultTone || 'Balanceado').toLowerCase()}:
                      </p>
                  ...
                  <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() =>
                          onToggleAccount(
                            accountDetails?.id || account.platform || account.id,
                    ...
                    {!showDisconnectConfirm ? (
                      ...
                      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                        <p className="text-sm text-red-800 dark:text-red-300 mb-4">
                          ¿Estás seguro? Esta acción eliminará permanentemente la conexión con{' '}
                          {account.handle}y todos los datos asociados.
                        </p>
