...
import {
  ...
  clearClipboard
} from '../utils/sensitiveDataDetector';
...
const AjustesSettings = ({ user, onNotification }) => {
      ...
      if (result?.data?.success) {
        setRoastrPersona((prev) => ({
          ...
          [field === 'identity'
            ...
            : field === 'intolerance'
              ? 'loQueNoTolero'
              : 'loQueMeDaIgual']: trimmedValue,
          [field === 'identity'
            ? 'isVisible'
            : field === 'intolerance'
              ? 'isIntoleranceVisible'
              : 'isToleranceVisible']: visibility,
  ...
  const performClipboardCopy = async (textToCopy, detection) => {
    try {
        ...
        if (navigator.permissions) {
          try {
            ...
            if (permission.state === 'denied') {
              onNotification?.(
                'Permisos de portapapeles denegados. Intentando método alternativo…',
                'warning'
              );
      ...
      if (error.name === 'NotAllowedError') {
        onNotification?.(
          'Permisos de portapapeles denegados. Intenta copiar manualmente.',
          'error'
        );
      } else if (error.name === 'SecurityError') {
        onNotification?.(
          'Error de seguridad al acceder al portapapeles. Intenta copiar manualmente.',
          'error'
        );
  ...
  const handleSensitiveModalConfirm = () => {
    if (copyState.pendingCopyData) {
      performClipboardCopy(copyState.pendingCopyData.text, copyState.pendingCopyData.detection);
    }
    setCopyState((prev) => ({
      ...prev,
      showSensitiveModal: false,
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-start space-x-2">
                ...
                <div className="text-sm text-blue-800">
                  <p className="font-medium mb-2">¿Por qué configurar tu Roastr Persona?</p>
                  ...
                  <p className="text-xs bg-blue-100 rounded p-2 border border-blue-300">
                    <strong>🔒 Privacidad garantizada:</strong> Todos los datos están encriptados
                    con AES-256 y son privados por defecto. Solo tú puedes verlos y decidir si
                    hacerlos públicos.
                  </p>
                </div>
              </div>
            ...
            <RoastrPersonaField
              ...
              onSave={(value, visibility) =>
                handleRoastrPersonaSave('intolerance', value, visibility)
              }
              onToggleForm={() =>
                setRoastrPersona((prev) => ({
                  ...prev,
            ...
            <RoastrPersonaField
              ...
              isSaving={roastrPersona.isSaving}
              onSave={(value, visibility) =>
                handleRoastrPersonaSave('tolerance', value, visibility)
              }
              onToggleForm={() =>
                setRoastrPersona((prev) => ({
                  ...prev,
              ...
              <div className="flex items-start space-x-2">
                ...
                <div className="text-sm text-amber-800">
                  <p className="mb-2">
                    ...
                    respuestas son generadas por IA. No es obligatorio, sino una opción de
                    personalización.
                  </p>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-gray-600 mt-1">"{copyState.bioText}"</div>
                  </div>
                  <div className="flex items-center space-x-2">
                  ...
                  <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg p-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-2">
                        ...
                        <div className="text-sm text-amber-800">{copyState.warningMessage}</div>
                      </div>
                      <div className="flex items-center space-x-2">
                        {isClipboardClearingSupported() && (
                          <Button
    ...
    <div className="border border-gray-200 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button variant="outline" size="sm" onClick={onToggleForm} disabled={isSaving}>
      ...
      {showForm && (
        <div className="space-y-3">
          <p className="text-xs text-gray-600 bg-gray-50 rounded p-2">{getFieldDescription()}</p>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span
                className={`text-xs ${fieldValue.length > maxLength ? 'text-red-600 font-medium' : 'text-gray-500'}`}
              >
      ...
      {!showForm && value && (
        <div className="text-sm text-gray-600 bg-gray-50 rounded p-3">{value}</div>
      )}
    </div>
  );
};
        ...
        <div className="mt-6 space-y-4" role="region" aria-label="Configuración de Shield">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Estado de Shield</label>
              <div className="flex items-center space-x-3">
                <button
                  className={`w-12 h-6 rounded-full relative transition-colors ${
                    shieldSettings.enabled ? 'bg-green-500' : 'bg-gray-300'
                  }`}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Nivel de agresividad</label>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t">
            ...
            <Button size="sm">Configuración avanzada</Button>
          </div>
        </div>
      )}
    </div>
