...
export default function AnalysisUsageCard({ user, className = '' }) {
  ...
  if (loading) {
    return (
      <div className={`usage-card-analysis flex items-center justify-between p-4 w-full md:w-96 h-24 flex-shrink-0 ${className}`}>
        <div>
  ...
  if (error) {
    return (
      <div className={`usage-card-error flex items-center justify-between p-4 w-full md:w-96 h-24 flex-shrink-0 ${className}`}>
        <div>
