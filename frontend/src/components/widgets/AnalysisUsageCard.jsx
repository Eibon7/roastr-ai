...
export default function AnalysisUsageCard({ user, className = '' }) {
  ...
  if (loading) {
    return (
      <div className={`w-[375px] h-[93px] border-l-8 border-l-yellow-500 border-b border-b-gray-200 flex flex-col justify-center p-4 flex-shrink-0 ${className}`}>
  ...
  return (
    <div className={`w-[375px] h-[93px] border-l-8 border-l-yellow-500 border-b border-b-gray-200 flex flex-col justify-center p-4 flex-shrink-0 ${className}`}>
      ...
      <p className="text-xs text-gray-500">
        {analysisUsed.toLocaleString()}/{analysisLimit === -1 ? '∞' : analysisLimit.toLocaleString()} comments ({percentageUsed}%)
      </p>
    </div>
  );
}
