...
import { Brain, Zap, Calendar, TrendingUp, AlertTriangle, Crown, ExternalLink } from 'lucide-react';
...
export default function CreditsCard() {
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-4">
            ...
            <p className="text-sm text-muted-foreground">Unable to load credits status</p>
            <Button variant="outline" size="sm" onClick={fetchCredits} className="mt-2">
              Retry
            </Button>
          </div>
        </CardContent>
  ...
  const analysisPercentage = credits?.analysis
    ? (credits.analysis.used / credits.analysis.limit) * 100
    : 0;
  const roastPercentage = credits?.roast ? (credits.roast.used / credits.roast.limit) * 100 : 0;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className={`text-sm font-medium ${getUsageColor(analysisPercentage)}`}>
                {credits?.analysis?.used?.toLocaleString() || 0} /{' '}
                {credits?.analysis?.limit?.toLocaleString() || 0}
              </div>
          ...
          {shouldShowUpgrade('analysis') && (
            <div className="flex items-center gap-2 p-2 bg-yellow-50 rounded-lg border border-yellow-200">
              ...
              <Button size="sm" variant="outline" onClick={handleUpgrade} className="text-xs h-6">
                Upgrade
              </Button>
            </div>
          )}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className={`text-sm font-medium ${getUsageColor(roastPercentage)}`}>
                {credits?.roast?.used?.toLocaleString() || 0} /{' '}
                {credits?.roast?.limit?.toLocaleString() || 0}
              </div>
              <div className="text-xs text-muted-foreground">
          ...
          {shouldShowUpgrade('roast') && (
            <div className="flex items-center gap-2 p-2 bg-yellow-50 rounded-lg border border-yellow-200">
              <AlertTriangle className="h-4 w-4 text-yellow-600" />
              <span className="text-xs text-yellow-700 flex-1">Running low on roast credits</span>
              <Button size="sm" variant="outline" onClick={handleUpgrade} className="text-xs h-6">
                Upgrade
              </Button>
            </div>
        ...
        <div className="pt-3 border-t space-y-3">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium">
              {credits?.period_end ? new Date(credits.period_end).toLocaleDateString() : 'Unknown'}
            </span>
          </div>
